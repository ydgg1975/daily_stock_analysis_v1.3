...
describe('stockIndexLoader', () => {
  ...
  describe('loadStockIndex - Load stock index', () => {
    ...
    test('successfully loads compressed format index (tuple format)', async () => {
      const compressedData = [
        ['600519.SH', '600519', 'guizhoumaotai', 'guizhoumaotai', 'gzmt', ['maotai'], 'CN', 'stock', true, 100],
        ['000001.SZ', '000001', 'pinganyinhang', 'pinganyinxing', 'payh', ['pingyin'], 'CN', 'stock', true, 90],
      ];
      ...
      expect(result.data[0].nameZh).toBe('guizhoumaotai');
    });
  ...
  describe('findStockInIndex - Find stock', () => {
    test('finds existing stock', () => {
      ...
      expect(result?.nameZh).toBe('guizhoumaotai');
    });
  ...
  describe('Edge case comprehensive tests', () => {
    ...
    test('handles special characters', () => {
      ...
      expect(compressed[0][2]).toBe('ceshi·gongsi');
      expect(compressed[0][5]).toEqual(['ceshi(jituan)']);
    });
  });
});
