...
describe('normalizeQuery', () => {
  describe('normalizeQuery - Query normalization', () => {
    test('removes leading and trailing spaces', () => {
      ...
      expect(normalizeQuery('  maotai  ')).toBe('maotai');
    });
    ...
    test('normalizes full-width latin characters to ASCII', () => {
      expect(normalizeQuery('wankeＡ')).toBe('wankea');
  ...
  describe('isChineseChar - Chinese character detection', () => {
    test('identifies Chinese characters', () => {
      expect(isChineseChar('mao')).toBe(true);
      expect(isChineseChar('tai')).toBe(true);
      expect(isChineseChar('gu')).toBe(true);
    });
  ...
  describe('containsChinese - Contains Chinese detection', () => {
    test('pure Chinese strings', () => {
      expect(containsChinese('guizhoumaotai')).toBe(true);
      expect(containsChinese('tengxun')).toBe(true);
    });

    test('mixed Chinese-English strings', () => {
      expect(containsChinese('600519guizhoumaotai')).toBe(true);
      expect(containsChinese('AAPLpingguo')).toBe(true);
    });
  ...
  describe('isStockCodeLike - Check if looks like stock code', () => {
    ...
    test('rejects Chinese names', () => {
      expect(isStockCodeLike('guizhoumaotai')).toBe(false);
      expect(isStockCodeLike('tengxun')).toBe(false);
    });
  ...
  describe('isStockNameLike - Check if looks like stock name', () => {
    test('identifies Chinese names', () => {
      expect(isStockNameLike('guizhoumaotai')).toBe(true);
      expect(isStockNameLike('tengxunkonggu')).toBe(true);
      expect(isStockNameLike('pinganyinhang')).toBe(true);
    });
    ...
    test('identifies mixed Chinese-English', () => {
      expect(isStockNameLike('guizhoumaotai600519')).toBe(true);
      expect(isStockNameLike('AAPLpingguo')).toBe(true);
    });
  ...
  describe('isPinyinLike - Check if looks like pinyin', () => {
    ...
    test('rejects Chinese characters', () => {
      expect(isPinyinLike('maotaimaotai')).toBe(false);
      expect(isPinyinLike('guizhou')).toBe(false);
    });
  ...
  describe('Edge case comprehensive tests', () => {
    ...
    test('special Unicode characters', () => {
      expect(normalizeQuery('gupiao🚀')).toBe('gupiao🚀');
