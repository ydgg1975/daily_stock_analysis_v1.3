...
describe('ChatPage', () => {
  ...
  it('switches session when clicking anywhere on the session card', async () => {
    ...
    const sessionCard = await screen.findByRole('button', {
      name: /qiehuandaoduihua qingjianyaofenxi 600519/,
    });
  ...
  it('renders a separate delete button for each session and opens confirmation without switching', async () => {
    ...
    const deleteButton = await screen.findByRole('button', {
      name: /shanchuduihua qingjianyaofenxi 600519/,
    });

    fireEvent.click(deleteButton);

    expect(mockSwitchSession).not.toHaveBeenCalled();
    expect(await screen.findByText('shanchuhou，gaiduihuajiangbukehuifu，querenshanchuma？')).toBeInTheDocument();
  });
  ...
  it('hides header actions when there are no messages', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'wengu' })).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'daochuhuihua' })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'fasongdaoyipeizhidetongzhijiqiren/youxiang' })).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'lishiduihua' })).toBeInTheDocument();
  });
  ...
  it('exports the current session from the header action', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: 'daochuhuihuawei Markdown wenjian' }));
  ...
  it('renders assistant skill labels with shared badge semantics', async () => {
    ...
    const skillBadge = await screen.findByLabelText('jineng qushifenxi');
  ...
  it('renders assistant multi-skill labels with shared badge semantics', async () => {
    ...
    const skillBadge = await screen.findByLabelText('jineng qushifenxi、junxianjincha');
    expect(skillBadge).toBeInTheDocument();
  ...
  it('selects the default skill after loading skills', async () => {
    ...
    expect(screen.getByRole('checkbox', { name: 'tongyongfenxi' })).not.toBeChecked();
  });
  ...
  it('sends multiple selected skills in order', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/fenxi 600519/), {
      target: { value: 'fenxi 600519' },
    });
    fireEvent.click(screen.getByRole('button', { name: 'fasong' }));
  ...
  it('omits skills when all concrete skills are cleared', async () => {
    ...
    expect(screen.getByRole('checkbox', { name: 'tongyongfenxi' })).toBeChecked();
  ...
  it('quick questions override the current multi-skill selection', async () => {
    ...
    fireEvent.click(await screen.findByRole('checkbox', { name: 'junxianjincha' }));
    fireEvent.click(screen.getByRole('button', { name: 'yongchanlunfenximaotai' }));

    await waitFor(() => {
      expect(mockStartStream).toHaveBeenCalledWith(
        expect.objectContaining({
          message: 'yongchanlunfenximaotai',
          skills: ['chan_theory'],
        }),
        expect.objectContaining({
          skillNames: ['chanlun'],
  ...
  it('keeps assistant message actions directly activatable in the DOM', async () => {
    ...
    const exportButton = await screen.findByRole('button', { name: 'daochucitiaoxiaoxiwei Markdown' });
  ...
  it('sends exported markdown to notification channel and shows success feedback', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: 'fasongdaoyipeizhidetongzhijiqiren/youxiang' }));
    ...
    expect(await screen.findByText('yifasongdaotongzhiqudao')).toBeInTheDocument();
  });
  ...
  it('prevents duplicate notification sends while the request is in flight', async () => {
    ...
    const sendButton = await screen.findByRole('button', { name: 'fasongdaoyipeizhidetongzhijiqiren/youxiang' });
  ...
  it('allows sending with base follow-up context before report hydration completes', async () => {
    ...
    expect(await screen.findByDisplayValue('qingshenrufenxi guizhoumaotai(600519)')).toBeInTheDocument();

    const sendButton = screen.getByRole('button', { name: /fasong|chulizhong\.\.\./ });
    expect(sendButton).not.toBeDisabled();
    expect(screen.getByText('zhengzaijiazailishifenxishangxiawen；xianzaikezhijiefasongzhuiwen。')).toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(mockStartStream).toHaveBeenCalledWith(
        expect.objectContaining({
          message: 'qingshenrufenxi guizhoumaotai(600519)',
          context: {
            stock_code: '600519',
  ...
  it('falls back to base stock context when recordId is missing', async () => {
    ...
    expect(await screen.findByDisplayValue('qingshenrufenxi AAPL')).toBeInTheDocument();
  ...
  it('ignores malformed follow-up query params', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'wengu' })).toBeInTheDocument();
    expect(screen.getByPlaceholderText(/fenxi 600519/)).toHaveValue('');
    expect(historyApi.getDetail).not.toHaveBeenCalled();
  });
  ...
  it('reprocesses follow-up query params when navigating to the same chat route again', async () => {
    ...
    expect(await screen.findByDisplayValue('qingshenrufenxi Apple(AAPL)')).toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(mockStartStream).toHaveBeenCalledWith(
        expect.objectContaining({
          message: 'qingshenrufenxi Apple(AAPL)',
          context: expect.objectContaining({
            stock_code: 'AAPL',
            stock_name: 'Apple',
  ...
  it('shows a jump-to-latest action when new content arrives while the user is away from bottom', async () => {
    ...
    const jumpButton = await screen.findByRole('button', { name: 'chakanzuixinxiaoxi' });
