...
const PerformanceCard: React.FC<{ metrics: PerformanceMetrics; title: string }> = ({ metrics, title }) => (
  <Card variant="gradient" padding="md" className="animate-fade-in">
    ...
    <MetricRow label="Average simulated return" value={pct(metrics.avgSimulatedReturnPct)} />
    <MetricRow label="Average stock return" value={pct(metrics.avgStockReturnPct)} />
    ...
    <MetricRow label="Average days to first hit" value={metrics.avgDaysToFirstHit != null ? metrics.avgDaysToFirstHit.toFixed(1) : '--'} />
        ...
        <div className="flex max-w-5xl flex-wrap items-center gap-2">
          <div className="relative min-w-0 flex-[1_1_220px]">
            <input
              ...
              placeholder="Filter by ticker, or leave blank for all"
              disabled={isRunning}
          ...
          {isLoadingResults ? (
            ...
            <div className="animate-fade-in">
              <div className="backtest-table-toolbar">
                <div className="backtest-table-toolbar-meta">
                  ...
                  <span className="text-xs text-secondary-text">
                    {codeFilter.trim() ? `Filtered to ${codeFilter.trim()}` : 'All stocks'}
                    {evalDays ? ` · ${evalDays}-day window` : ''}
                    {analysisDateFrom ? ` · from ${analysisDateFrom}` : ''}
                    {analysisDateTo ? ` · to ${analysisDateTo}` : ''}
                  </span>
                </div>
                <span className="backtest-table-scroll-hint">Scroll sideways on small screens</span>
              </div>
                      ...
                      >
                        <td className="backtest-table-cell backtest-table-code">
                          <div className="flex flex-col">
                            <span>{row.code}</span>
                            <span className="text-xs text-muted-text">{toEnglishText(row.stockName, '--')}</span>
                          </div>
                        </td>
                        ...
                        <td className="backtest-table-cell max-w-[220px] text-foreground">
                          {(row.trendPrediction || row.operationAdvice) ? (
                            <Tooltip
                              content={[row.trendPrediction, row.operationAdvice].filter(Boolean).map((value) => toEnglishText(value, '')).join(' / ')}
                              focusable
                            >
                              <div className="flex flex-col gap-1">
                                <span className="block truncate">{toEnglishText(row.trendPrediction, '--')}</span>
                                <span className="block truncate text-xs text-secondary-text">{toEnglishText(row.operationAdvice, '--')}</span>
                              </div>
                            </Tooltip>
                          ) : (
                            '--'
              ...
              <p className="text-xs text-muted-text text-center mt-2">
                {totalResults} results · Page {currentPage} / {Math.max(totalPages, 1)}
              </p>
            </div>
          )}
        </section>
