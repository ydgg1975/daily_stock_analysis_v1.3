  ...
  return (
    <div className="theme-page-transition backtest-v1-page workspace-page--backtest" data-testid="backtest-v1-page">
      <WorkspacePageHeader
        eyebrow="WolfyStock"
        title="回测"
        description={`配置页现在只负责发起 deterministic backtest，不再内嵌完整结果分析。普通模式提供引导式配置，专业模式提供密集控制；完整 KPI 与图表统一打开到独立结果页。当前基准默认按市场自动选择（${getBenchmarkModeLabel(ruleBenchmarkMode, normalizedCode, ruleBenchmarkCode)}）。`}
