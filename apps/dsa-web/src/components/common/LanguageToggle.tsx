...
export const LanguageToggle: React.FC<LanguageToggleProps> = ({
  ...
  return (
    <button
      ...
      className={cn(
        isNavVariant
          ? 'theme-panel-subtle flex h-12 w-full items-center gap-3 rounded-[1.35rem] px-4 text-sm text-secondary-text transition-all duration-200 ease-out hover:text-foreground'
      ...
      {isNavVariant ? (
        collapsed ? null : (
          <div className="min-w-0 text-left">
            <span className="block truncate text-[1.02rem] font-medium">{t('nav.language')}</span>
