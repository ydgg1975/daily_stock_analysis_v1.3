...
describe('Input', () => {
  ...
  it('renders a trailing action when provided', () => {
    render(
      <Input
        label="Password"
        name="password"
        trailingAction={<button type="button">표시</button>}
      />
    );
  ...
  it('toggles password visibility in uncontrolled mode', () => {
    render(<Input label="비밀번호" type="password" allowTogglePassword />);
  ...
  it('supports controlled password visibility', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '내용 숨기기' }));
  ...
  it('supports the login appearance without affecting password toggle behavior', () => {
    render(<Input label="로그인 비밀번호" type="password" allowTogglePassword appearance="login" />);

    const input = screen.getByLabelText('로그인 비밀번호');
