...
describe('IntelligentImport', () => {
  ...
  it('opens the matching hidden file input when the picker buttons are clicked', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'xuanzetupian' }));
    fireEvent.click(screen.getByRole('button', { name: 'xuanzewenjian' }));
  ...
  it('refreshes config state after a config version conflict', async () => {
    ...
    update.mockRejectedValue(
      new SystemConfigConflictError('peizhibanbenchongtu', 'v2'),
    );
    ...
    fireEvent.change(screen.getByPlaceholderText('huozhantie CSV/Excel fuzhidewenben...'), {
      target: { value: '000001' },
    });
    fireEvent.click(screen.getByRole('button', { name: 'jiexi' }));

    await screen.findByText('SZ000001');

    fireEvent.click(screen.getByRole('button', { name: 'hebingdaozixuangu' }));
    ...
    expect(await screen.findByText('peizhiyigengxin，qingzaicidianji「hebingdaozixuangu」')).toBeInTheDocument();
  });
});
