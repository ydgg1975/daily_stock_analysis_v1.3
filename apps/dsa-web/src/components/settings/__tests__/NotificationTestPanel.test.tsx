...
describe('NotificationTestPanel', () => {
  ...
  it('submits draft notification items and renders attempt details', async () => {
    ...
    fireEvent.change(screen.getByLabelText('qudao'), { target: { value: 'custom' } });
    fireEvent.click(screen.getByRole('button', { name: /fasongceshi/ }));
    ...
    expect(await screen.findByText('ceshichenggong')).toBeInTheDocument();
  ...
  it('renders custom webhook partial failure attempts', async () => {
    testNotificationChannel.mockResolvedValueOnce({
      success: true,
      message: 'zidingyi Webhook tongzhiceshibufenchenggong（1/2）',
      errorCode: null,
      stage: 'notification_send',
      retryable: true,
      latencyMs: 35,
    ...
    expect(await screen.findByText('ceshichenggong')).toBeInTheDocument();
    expect(screen.getByText(/bufenchenggong/)).toBeInTheDocument();
  ...
  it('renders retryable timeout diagnostics', async () => {
    testNotificationChannel.mockResolvedValueOnce({
      success: false,
      message: 'tongzhiceshiyichang: timeout',
      errorCode: 'timeout',
      stage: 'notification_send',
      retryable: true,
      latencyMs: null,
    ...
    expect(await screen.findByText('ceshishibai')).toBeInTheDocument();
