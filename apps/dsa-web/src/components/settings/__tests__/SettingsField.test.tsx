...
describe('SettingsField', () => {
  it('renders sensitive field metadata and validation errors', () => {
    ...
    expect(screen.getByText('mingan')).toBeInTheDocument();
    expect(screen.getByText('API Key bitian')).toBeInTheDocument();
  ...
  it('renders multi-value sensitive fields with external delete actions', () => {
    ...
    expect(screen.getAllByRole('button', { name: 'xianshineirong' })).toHaveLength(2);
    expect(screen.getAllByRole('button', { name: 'shanchu' })).toHaveLength(2);
  });
  ...
  it('allows optional select fields to be cleared when schema provides an empty option', () => {
    ...
    expect(screen.queryByRole('option', { name: 'qingxuanze' })).not.toBeInTheDocument();
  ...
  it('renders localized custom webhook body template guidance', () => {
    ...
    expect(screen.getByLabelText('zidingyi Webhook Body muban')).toBeInTheDocument();
    expect(screen.getByText(/huixianyu Bark、Slack、Discord dengzidong payload shengxiao/)).toBeInTheDocument();
    expect(screen.getByText(/luo \$content \/ \$title buzuo JSON zhuanyi/)).toBeInTheDocument();
  });
  ...
  it('opens detailed field help when help metadata is available', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'chakan zixuanguliebiao peizhishuoming' }));

    expect(screen.getByRole('dialog', { name: 'zixuanguliebiao' })).toBeInTheDocument();
    expect(screen.getByText('STOCK_LIST=600519,300750,002594')).toBeInTheDocument();
    const docLink = screen.getByRole('link', { name: /wanzhengzhinan/ });
    expect(docLink).toHaveAttribute('href', 'https://example.com/full-guide');

    const closeButtons = screen.getAllByRole('button', { name: 'guanbipeizhishuoming' });
    ...
    expect(screen.queryByRole('dialog', { name: 'zixuanguliebiao' })).not.toBeInTheDocument();
  });
});
