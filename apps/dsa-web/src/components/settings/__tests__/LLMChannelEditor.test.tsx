...
describe('LLMChannelEditor', () => {
  ...
  it('renders API Key input with controlled visibility', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '내용 표시' }));
  ...
  it('shows help dialogs for channel editor fields', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: 'Base URL 설정 설명 보기' }));
    ...
    fireEvent.click(await screen.findByRole('button', { name: 'Temperature 설정 설명 보기' }));
  ...
  it('hides LiteLLM wording when advanced YAML routing is enabled', () => {
    ...
    expect(screen.getByText(/고급 모델 라우팅 YAML/i)).toBeInTheDocument();
    expect(screen.getByText(/런타임 기본 모델 \/ 대체 모델 \/ Vision \/ Temperature/i)).toBeInTheDocument();
  ...
  it('keeps minimax-prefixed models in runtime selections', () => {
    ...
    const primaryModelSelect = screen.getByRole('combobox', { name: '기본 모델' });
    const agentModelSelect = screen.getByRole('combobox', { name: 'Agent 기본 모델' });
    const visionModelSelect = screen.getByRole('combobox', { name: 'Vision 모델' });
  ...
  it('uses DeepSeek V4 defaults when adding the official preset', async () => {
    ...
    await screen.findByRole('button', { name: /DeepSeek/i });
    expect(screen.getByLabelText('Base URL')).toHaveValue('https://api.deepseek.com');

    expect(screen.getByLabelText('모델(쉼표 구분)')).toHaveValue('deepseek-v4-flash,deepseek-v4-pro');
  });
  ...
  it.each([
    ...
    ['minimax', /MiniMax/i, 'https://api.minimax.io/v1', 'MiniMax-M2.7,MiniMax-M2.7-highspeed'],
    ...
    expect(screen.getByLabelText('모델(쉼표 구분)')).toHaveValue(models);
  ...
  it('shows provider capability badges, official sources, and config hints', async () => {
    ...
    expect(await screen.findByText('설정 참고')).toBeInTheDocument();

    expect(screen.getByText('OpenAI 호환')).toBeInTheDocument();

    expect(screen.getByText('통합 플랫폼')).toBeInTheDocument();

    expect(screen.getByText('모델 가져오기 가능')).toBeInTheDocument();
    ...
    expect(screen.getByText(/기능 태그는 설정 참고용이며 런타임 기능 검증 통과를 의미하지 않습니다/i)).toBeInTheDocument();
  ...
  it('shows model-discovery capability for SiliconFlow provider hints', async () => {
    ...
    expect(await screen.findByText('모델 가져오기 가능')).toBeInTheDocument();
  ...
  it('does not show provider metadata for custom or unknown channels', async () => {
    ...
    expect(screen.queryByText('설정 참고')).not.toBeInTheDocument();

    expect(screen.queryByText(/공식 출처/i)).not.toBeInTheDocument();

    expect(screen.queryByText(/기능 태그는 설정 참고용/i)).not.toBeInTheDocument();
  ...
  it('preserves manually edited base URL and models when switching preset names', async () => {
    ...
    fireEvent.change(screen.getByLabelText('모델(쉼표 구분)'), {

      target: { value: 'custom-model-a,custom-model-b' },

    });

    fireEvent.change(screen.getByLabelText('채널 이름'), {
    ...
    await screen.findByRole('button', { name: /MiniMax/i });

    expect(screen.getByLabelText('Base URL')).toHaveValue('https://proxy.example.com/v1');

    expect(screen.getByLabelText('모델(쉼표 구분)')).toHaveValue('custom-model-a,custom-model-b');
  ...
  it('uses the selected preset defaults when adding a duplicate provider channel', async () => {
    ...
    expect(screen.getAllByLabelText('채널 이름').map((input) => (input as HTMLInputElement).value)).toEqual([
    ...
    expect(screen.getAllByLabelText('모델(쉼표 구분)').map((input) => (input as HTMLInputElement).value)).toEqual([
  ...
  it('shows cleanup warning and restore path after stale runtime models are removed on save', async () => {
    ...
    update.mockResolvedValue({
      ...
      warnings: [
        ...
        '동기화 중 사라진 런타임 모델 참조를 정리했습니다: 기본 모델 / Agent 기본 모델 / Vision 모델 / 대체 모델 항목. 복구가 필요하면 채널 모델 목록을 되돌린 뒤 다시 선택하거나 데스크톱 진단 백업 또는 .env에서 이전 LLM_* / LITELLM_MODEL / AGENT_LITELLM_MODEL / VISION_MODEL / LLM_TEMPERATURE 값을 복원하세요.',
    ...
    expect(await screen.findByText('저장 후 안내')).toBeInTheDocument();

    expect(screen.getByText(/사라진 런타임 모델 참조를 정리/i)).toBeInTheDocument();

    expect(screen.getByText(/데스크톱 진단 백업 또는 \.env에서 이전/i)).toBeInTheDocument();
  ...
  it('keeps save warnings visible after onSaved-driven refresh', async () => {
    ...
    const warningMessage = '동기화 중 사라진 런타임 모델 참조를 정리했습니다: 기본 모델 / Agent 기본 모델 / Vision 모델 / 대체 모델 항목.';
  ...
  it('keeps runtime selections while channel models are edited temporarily', async () => {
    ...
    const modelInput = screen.getByLabelText('모델(쉼표 구분)');
  ...
  it('checks protocol-prefixed selected model when discovery returns bare id', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Qwen DashScope/i }));

    fireEvent.click(screen.getByRole('button', { name: '모델 가져오기' }));
    ...
    await waitFor(() => {
      ...
      expect(screen.getByLabelText('수동 모델(쉼표 구분)')).toHaveValue('');
  ...
  it('does not treat unknown-prefixed selected model as equivalent to bare discovered id', async () => {
    ...
    expect(screen.getByLabelText('수동 모델(쉼표 구분)')).toHaveValue('minimax/MiniMax-M1');
  ...
  it('discovers models and writes selected values back to channel config', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByLabelText('수동 모델(쉼표 구분)')).toHaveValue('qwen-old,qwen-plus');
  ...
  it('shows structured troubleshooting hint when channel auth fails', async () => {
    ...
    expect(screen.queryByText(/모델 순서를 조정하거나 사용할 수 없는 모델을 제거/i)).not.toBeInTheDocument();
  ...
  it('shows tested model and model-availability hints when a model is disabled', async () => {
    ...
    expect(screen.getAllByText(/openai\/deepseek-ai\/DeepSeek-V3/i).length).toBeGreaterThan(0);

    expect(screen.getByText(/기본 연결 테스트는 모델 목록의 첫 번째 모델만 테스트합니다/i)).toBeInTheDocument();

    expect(screen.getByText(/기본 연결 테스트는 모델 목록의 첫 번째 모델만 테스트합니다/i)).toBeInTheDocument();

    expect(screen.getByText(/모델 순서를 조정하거나 사용할 수 없는 모델을 제거/i)).toBeInTheDocument();

    expect(screen.getByText(/모델 사용 권한|계정으로 해당 모델을 사용/i)).toBeInTheDocument();
  ...
  it('shows provider blocked troubleshooting without network or model-list hints', async () => {
    ...
    expect(await screen.findByText(/요청 차단됨/i)).toBeInTheDocument();

    expect(screen.getAllByText(/openai\/gpt-5\.5/i).length).toBeGreaterThan(0);

    expect(screen.getByText(/계정 리스크 제어, 지역 제한, 모델 권한/i)).toBeInTheDocument();
  ...
  it('shows focused quota exceeded troubleshooting hints', async () => {
    ...
    expect((await screen.findAllByText(/한도|소진|quota|balance|limit/i)).length).toBeGreaterThan(0);
  ...
  it('does not show model-list action hints for network failures', async () => {
    ...
    expect(await screen.findByText(/도메인 해석 실패/i)).toBeInTheDocument();
  ...
  it('does not request runtime capabilities during the basic connection test', async () => {
    ...
    await screen.findByText(/연결 성공 · openai\/gpt-4o-mini/i);
  ...
  it('runs explicit runtime capability checks and shows detailed hints', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '기능 검사' }));



    expect(await screen.findByText(/기능 검사 완료: 1 통과 \/ 1 실패 \/ 0 건너뜀/i)).toBeInTheDocument();

    expect(screen.getByText('JSON 통과')).toBeInTheDocument();

    expect(screen.getByText('Tools 실패')).toBeInTheDocument();

    expect(screen.getByText(/현재 모델 또는 호환 계층이 이 기능을 지원하지 않습니다/i)).toBeInTheDocument();
  ...
  it('shows skipped runtime capabilities when the base test fails', async () => {
    ...
    expect(await screen.findByText(/기능 검사 완료: 0 통과 \/ 0 실패 \/ 1 건너뜀/i)).toBeInTheDocument();

    expect(screen.getByText('JSON 건너뜀')).toBeInTheDocument();

    expect(screen.getByText(/제공업체가 현재 API Key를 거부했습니다/i)).toBeInTheDocument();

    expect(screen.getByLabelText('모델(쉼표 구분)')).toBeEnabled();
  ...
  it('keeps manual model input available when discovery fails', async () => {
    ...
    await screen.findByText(/Model discovery is not supported for this protocol/i);

    expect(screen.getByText(/현재 자동 모델 검색은 OpenAI Compatible \/ DeepSeek 채널에만 제공됩니다/i)).toBeInTheDocument();



    const manualInput = screen.getByLabelText('모델(쉼표 구분)');
  ...
  it('maps discovery format errors to the /models troubleshooting hint', async () => {
    ...
    expect(screen.getByText(/이 채널의 \/models 응답 형식이 호환되지 않습니다/i)).toBeInTheDocument();
  ...
  it('maps discovery empty responses to the /models troubleshooting hint', async () => {
    ...
    expect(await screen.findByText(/No model IDs returned/i)).toBeInTheDocument();

    expect(screen.getByText(/사용 가능한 모델 ID를 반환하지 않았습니다/i)).toBeInTheDocument();

    expect(screen.queryByText(/호환 모델로 바꾸거나 추가 응답 모드를 끈 뒤/i)).not.toBeInTheDocument();
  ...
  it('does not apply stale discovery response after channel list re-sync', async () => {
    ...
    expect(screen.getByLabelText('수동 모델(쉼표 구분)')).toHaveValue('dash-old,dashscope-plus');
  ...
  it('does not apply stale discovery response after inline channel edit', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByLabelText('모델(쉼표 구분)')).toHaveValue('qwen-old');
