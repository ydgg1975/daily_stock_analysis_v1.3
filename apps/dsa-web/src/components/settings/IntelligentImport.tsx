...
export const IntelligentImport: React.FC<IntelligentImportProps> = ({
  ...
  const handleImageFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        let fallback = 'Recognition failed. Please try again.';
        if (err?.response?.status === 429) fallback = 'Too many requests. Try again later.';
        else if (err?.code === 'ECONNABORTED') fallback = 'Request timed out. Check the network and retry.';
  ...
  const handleDataFile = useCallback(
    async (file: File) => {
      if (file.size > FILE_MAX) {
        setError('Files must be 2 MB or smaller.');
        return;
      }
      setError(null);
      setIsLoading(true);
      try {
        ...
        const parsed = getParsedApiError(e);
        setError(parsed.message || 'Parsing failed.');
      } finally {
        setIsLoading(false);
      }
    },
  ...
  const handlePasteParse = useCallback(() => {
    ...
    if (new Blob([t]).size > TEXT_MAX) {
      setError('Pasted text must be 100 KB or smaller.');
      return;
    }
    setError(null);
    setIsLoading(true);
  ...
  const mergeToWatchlist = useCallback(async () => {
    ...
    if (!configVersion) {
      setError('Load settings before merging.');
      return;
    }
    ...
    try {
      ...
      if (e instanceof SystemConfigConflictError) {
        await onMerged(value);
        setError('Settings were updated. Click Merge to Watchlist again.');
      } else {
        setError(e instanceof Error ? e.message : 'Merge save failed.');
      }
    } finally {
      setIsMerging(false);
    }
  ...
  return (
    <div className="space-y-4">
      <div className="settings-surface-panel settings-border-strong rounded-xl border p-4 shadow-soft-card">
        <p className="text-sm font-medium text-foreground">Supports images, CSV/Excel files, and clipboard text</p>
        <p className="mt-1 text-xs leading-5 text-secondary-text">
          Image recognition requires a configured Vision model. Review parsed results manually before merging them into the watchlist.
        </p>
      </div>
      ...
      >
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <textarea
            placeholder="Or paste text copied from CSV/Excel..."
      ...
      {items.length > 0 && (
        <div className="space-y-2">
          <InlineAlert
            variant="warning"
            message="Review each item before merging. High-confidence items are selected by default; medium/low-confidence items need manual confirmation."
            className="rounded-xl px-3 py-2 text-xs shadow-none"
          />
          <div className="flex items-center justify-between">
            <span className="text-xs text-secondary-text">
              {validCount} mergeable item(s), {checkedCount} selected
            </span>
