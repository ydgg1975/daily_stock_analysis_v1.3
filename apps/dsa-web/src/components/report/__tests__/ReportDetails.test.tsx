...
describe('ReportDetails', () => {
  ...
  it('keeps copied feedback scoped to the panel that was copied', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'yuanshifenxijieguo' }));
    fireEvent.click(screen.getByRole('button', { name: 'fenxikuaizhao' }));

    const [rawCopyButton, snapshotCopyButton] = screen.getAllByRole('button', { name: 'fuzhi' });
    ...
    expect(screen.getByRole('button', { name: 'yifuzhi' })).toBeInTheDocument();
    expect(screen.getAllByRole('button', { name: 'fuzhi' })).toHaveLength(1);
    ...
    expect(screen.getAllByRole('button', { name: 'yifuzhi' })).toHaveLength(2);

    act(() => {
      vi.advanceTimersByTime(2000);
    });

    expect(screen.getAllByRole('button', { name: 'fuzhi' })).toHaveLength(2);
  });
