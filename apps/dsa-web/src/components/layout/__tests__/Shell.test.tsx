...
describe('Shell', () => {
  it.skip('renders navigation, theme toggle and completion badge', () => {
    ...
    expect(screen.getAllByRole('button', { name: '테마 전환' }).length).toBeGreaterThan(0);
    expect(screen.getByRole('link', { name: 'AI 종목 상담' })).toBeInTheDocument();
  ...
  it.skip('opens the theme menu from the sidebar toggle', async () => {
    ...
    fireEvent.click(screen.getAllByRole('button', { name: '테마 전환' })[0]);
