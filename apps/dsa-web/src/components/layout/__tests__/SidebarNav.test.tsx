...
const mockThemeToggle = vi.fn(({ collapsed }: { collapsed?: boolean }) => (
  <button type="button">{collapsed ? 'qiehuanzhuti(zhedie)' : 'qiehuanzhuti'}</button>
));
...
describe('SidebarNav', () => {
  it('shows the shared completion badge only when chat completion is pending', () => {
    ...
    expect(screen.getByLabelText('AI 종목 상담 새 메시지')).toBeInTheDocument();
  ...
  it('renders the collapsed theme toggle variant when the sidebar is collapsed', () => {
    ...
    expect(screen.getByRole('button', { name: 'qiehuanzhuti(zhedie)' })).toBeInTheDocument();
  });
  ...
  it('opens the logout confirmation and confirms logout', async () => {
    ...
    expect(await screen.findByRole('heading', { name: '로그아웃' })).toBeInTheDocument();
