...
try:
    ...
    logger.warning("[DingTalk Stream] dingtalk-stream SDK is not installed，Stream mode is unavailable")
...
class DingtalkStreamClient:
    ...
    def __init__(
        ...
        if not self._client_id or not self._client_secret:
            raise ValueError(
                "钉钉 Stream 模式requires configuration DINGTALK_APP_KEY 和 DINGTALK_APP_SECRET"
            )
    ...
    def _run_in_background(self) -> None:
        ...
        while self._running:
            try:
                ...
                if self._running:
                    logger.info("[DingTalk Stream] Reconnect after 5 seconds...")
                    time.sleep(5)
