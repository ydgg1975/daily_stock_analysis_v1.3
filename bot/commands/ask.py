...
Usage:
    ...
    /ask 600519 yongchanlunanalysis              -> Parse skill from message

    /ask 600519 chan_theory             -> Specify skill id directly

    /ask 600519,000858 bolanglilun         -> Multi-stock comparison with skill overlay
...
class AskCommand(BotCommand):
    ...
    def description(self) -> str:
        ...
        return "shiyong Agent jinenganalysisstock"
    ...
    def usage(self) -> str:
        ...
        return "/ask <stockdaima[,daima2,...]> [jinengmingcheng]"
    ...
    def _validate_single_code(self, code: str) -> Optional[str]:
        ...
        if not (is_a_stock or is_hk_stock or is_us_stock):
            ...
            return f"Invalid stock code: {normalized}. Use KR005930, HK00700, AAPL, or CN600519 style codes."
    ...
    def validate_args(self, args: List[str]) -> Optional[str]:
        ...
        if not args:
            ...
            return "inputrustockdaima?굖ongfa: /ask <stockdaima[,daima2,...]> [jinengmingcheng]"
        ...
        if not codes:
            ...
            return "inputruzhishaoyigeyouxiaodestockdaima"
        ...
        if len(codes) > 5:
            ...
            return "yicizuiduoanalysis 5 zhistock"
    ...
    def _build_user_message(stock_code: str, skill_id: str, skill_text: str) -> str:
        ...
        user_msg = f"qinganalysisstock {stock_code}"

        if skill_id:

            user_msg = f"qingshiyong {skill_id} jinenganalysisstock {stock_code}"

        if skill_text:

            user_msg = f"Analyze stock {stock_code} {skill_text}"
    ...
    def execute(self, message: BotMessage, args: List[str]) -> BotResponse:
        ...
        if not config.agent_mode:
            ...
            return BotResponse.text_response(
                ...
                "Agent mode is disabled. Set `AGENT_MODE=true` in config to use this command."
    ...
    def _analyze_single(
        ...
        try:
            ...
            if result.success:
                ...
                header = f"?뱤 {code} | jineng: {skill_name}\n{'?' * 30}\n"

                return BotResponse.text_response(header + result.content)

            return BotResponse.text_response(f"?좑툘 analysisshibai: {result.error}")
            ...
            return BotResponse.text_response(f"?좑툘 wenguzhixingchucuo: {str(exc)}")
    ...
    def _analyze_multi(
        ...
        def _run_one(stock_code: str) -> Tuple[str, Optional[Dict[str, Any]], Optional[str]]:
            ...
            try:
                ...
                return (stock_code, None, result.error or "weizhicuowu")
            ...
            for future in as_completed(future_map, timeout=overall_timeout_s):
                ...
                try:
                    ...
                    if content is not None:
                        ...
                        errors[code] = error or "weizhicuowu"

                except Exception as exc:

                    code = future_map[future]

                    errors[code] = f"zhixingyichang: {exc}"
        ...
        for code in codes:
            ...
            if code not in results and code not in errors:
                ...
                errors[code] = "analysischaoshi"



        parts = [f"?뱤 **duoguduibianalysis** | jineng: {skill_name}", f"{'?' * 30}", ""]
        ...
        if len(results) >= 2:
            ...
            parts.append("| stock | xinhao | zhixindu | zhaiyao |")
            ...
            for code in codes:
                ...
                if code in results:
                    ...
                    summary_line = str(item.get("summary") or "analysiswancheng").replace("|", "/")[:80]

                    parts.append(f"| {code} | {signal} | {confidence_text} | {summary_line} |")

                elif code in errors:

                    parts.append(f"| {code} | error | - | ?좑툘 {errors[code][:40]} |")
        ...
        for code in codes:
            ...
            if code in results:
                ...
                parts.append(f"?좑툘 analysisshibai: {errors[code]}")
    ...
    def _extract_confidence(dashboard: Optional[Dict[str, Any]]) -> Optional[float]:
        ...
        return {"gao": 0.85, "zhong": 0.65, "di": 0.45}.get(level)
    ...
    def _extract_summary(stock_code: str, dashboard: Optional[Dict[str, Any]], raw_content: str) -> str:
        ...
        return f"{stock_code} analysiswancheng"
    ...
    def _format_stock_result(stock_code: str, dashboard: Optional[Dict[str, Any]], raw_content: str) -> str:
        ...
        if isinstance(stock_name, str) and stock_name.strip() and stock_name.strip() != stock_code:
            ...
            lines.append(f"**mingcheng**: {stock_name.strip()}")
        ...
        if isinstance(decision, str):
            ...
            lines.append(
                ...
                + (f" | **zhixindu**: {confidence:.0%}" if isinstance(confidence, (int, float)) else "")

                + (f" | **qushi**: {trend}" if isinstance(trend, str) and trend.strip() else "")
        ...
        if summary:
            ...
            lines.append(f"**zhaiyao**: {summary}")
        ...
        if isinstance(operation, str) and operation.strip():
            ...
            lines.append(f"**caozuojianyi**: {operation.strip()}")
        ...
        if isinstance(risk_warning, str) and risk_warning.strip():
            ...
            lines.append(f"**fengxiantishi**: {risk_warning.strip()}")
        ...
        if isinstance(sniper, dict):
            ...
            if price_parts:
                ...
                lines.append("**guanjiandianwei**: " + " | ".join(price_parts))
    ...
    def _build_portfolio_section(
        ...
        def _render_overlay() -> str:
            ...
            if risk_score is not None:
                ...
                lines.append(f"- zuhefengxianfen: {risk_score}")
            ...
            if rebalance:
                ...
                lines.append(f"- Rebalance suggestion: {', '.join(str(item) for item in rebalance[:3])}")
            ...
            if positions:
                ...
                if position_parts:
                    ...
                    lines.append(f"- Suggested position: {', '.join(position_parts)}")
