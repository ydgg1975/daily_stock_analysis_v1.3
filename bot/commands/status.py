...
class StatusCommand(BotCommand):
    """Show system status and key configuration state."""
    ...
    def _format_status(self, status: dict, platform_name: str = "") -> str:
        ...
        if status["stock_list"]:
            ...
            if status["stock_count"] > 5:
                stocks_preview += f" ... 외 {status['stock_count'] - 5}개"
        ...
        if status["ai_available"]:
            lines.extend(["", "---", "✅ **시스템 준비 완료: 분석을 시작할 수 있습니다.**"])
        else:
            lines.extend(
                [
                    "",
                    ...
                    "LITELLM_MODEL, LLM_CHANNELS, LITELLM_CONFIG 또는 provider API Key를 설정하세요.",
                ]
            )
