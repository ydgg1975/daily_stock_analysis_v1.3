...
"""
===================================
LongbridgeFetcher - zhangqiaodoudishujuyuan (Priority 5)
===================================

shujulaiyuan：zhangqiao OpenAPI (https://open.longbridge.com)
tedian：fugaimeigu + ganggu，kejisuanliangbi/huanshoulv/PE deng yfinance queshiziduan
dingwei：meigu/gangguzuihoudoudishujuyuan

guanjiancelve：
1. zuhe quote + static_info jiekoujisuan turnover_rate / pe_ratio / total_mv
2. tongguo history_candlesticks jisuan volume_ratio（jin5rijunliangbi）
3. lanjiazai QuoteContext，shoucidiaoyongshicaijianlilianjie
4. static_info jinchengneiduanhuancun，jianshaochongfuqingqiu（moren 24h，kediao；jian LONGBRIDGE_STATIC_INFO_TTL_SECONDS）

pingzheng：`LONGBRIDGE_APP_KEY` / `LONGBRIDGE_APP_SECRET` / `LONGBRIDGE_ACCESS_TOKEN`。
kexuan：`LONGBRIDGE_STATIC_INFO_TTL_SECONDS`；SDK `language` quzi `REPORT_LANGUAGE`，`log_path` wei `{LOG_DIR}/longbridge_sdk.log`；
`LONGBRIDGE_HTTP_URL` / `LONGBRIDGE_QUOTE_WS_URL` / `LONGBRIDGE_TRADE_WS_URL` / `LONGBRIDGE_REGION` （jianguanfangwendangmorenzhi）。
"""
...
def _sanitize_longbridge_env() -> None:
    ...
    for key in (
        ...
        if val is not None and val.strip() == "":
            del os.environ[key]
            logger.debug("[Longbridge] shanchukonghuanjingbianliang %s", key)
    ...
    if not os.environ.get("LONGBRIDGE_LOG_PATH"):
        try:
            ...
            logger.debug("[Longbridge] shezhi LONGBRIDGE_LOG_PATH=%s",
    ...
    if region:
        if not os.environ.get("LONGPORT_REGION"):
            ...
            logger.debug("[Longbridge] tongbu LONGPORT_REGION=%s", region)
        ...
        for env_name, default_url in (
            ...
            if default_url and not os.environ.get(env_name):
                ...
                logger.debug("[Longbridge] genju REGION=%s shezhi %s=%s",
...
class LongbridgeFetcher(BaseFetcher):
    """
    zhangqiao OpenAPI shujuyuanshixian

    youxianji: 5（zuidi，zuoweimeigu/gangguzuihoudoudi）
    shujulaiyuan: Longbridge OpenAPI

    tongguozuheduoge API jisuan yfinance queshidezhibiao:
    ...
    def _mark_connection_cooldown(self, exc: Exception) -> None:
        ...
        logger.warning(
            "[Longbridge] jiancedaolianjieyichang，jinru %ss lengqueqiyibimianpinfanzhonglian: %s",
            cooldown_seconds,
            exc,
        )
    ...
    def is_available_for_request(self, capability: str = "") -> bool:
        ...
        if self._cooldown_until > time.time():
            logger.debug(
                "[Longbridge] %s lengquezhong，zanshitiaoguoqingqiu，shengyu %.1fs",
        ...
        with self._ctx_lock:
            ...
            try:
                ...
                for factory_name in ("from_apikey_env", "from_env"):
                    ...
                    try:
                        lb_config = factory()
                        logger.info("[Longbridge] Config.%s() chenggong", factory_name)
                        break
                    except Exception as e:
                        logger.debug(
                            "[Longbridge] Config.%s() shibai: %s", factory_name, e
                        )
                ...
                if lb_config is None:
                    ...
                    logger.info("[Longbridge] Config.from_apikey() chuangjianchenggong")

                # Diagnostic logging
                region = os.getenv("LONGBRIDGE_REGION") or os.getenv("LONGPORT_REGION") or "(auto)"
                logger.info(
                    "[Longbridge] peizhi: region=%s, http=%s, quote_ws=%s",
                    region,
                ...
                logger.info("[Longbridge] QuoteContext chushihuachenggong")
                return self._ctx
            except Exception as e:
                logger.warning("[Longbridge] QuoteContext chushihuashibai: %s", e)
    ...
    def _get_static_info(self, symbol: str) -> Optional[Any]:
        ...
        try:
            ...
            logger.debug(f"[Longbridge] static_info({symbol}) shibai: {e}")
    ...
    def _compute_volume_ratio(self, symbol: str, today_volume: int) -> Optional[float]:
        ...
        try:
            ...
            logger.debug(f"[Longbridge] jisuanliangbishibai({symbol}): {e}")
            return None
    ...
    def get_realtime_quote(self, stock_code: str) -> Optional[UnifiedRealtimeQuote]:
        ...
        symbol = _to_longbridge_symbol(stock_code)
        if symbol is None:
            logger.debug(f"[Longbridge] wufazhuanhuandaima: {stock_code}")
            return None
        ...
        try:
            ...
            logger.info(f"[Longbridge] quote({symbol}) shibai: {e}")
            if self._is_connection_error(e):
                self._mark_connection_cooldown(e)
            return None
        ...
        if static is not None:
            ...
            if shares_for_turnover > 0 and volume > 0:
                ...
                logger.debug(
                    "[Longbridge] %s wufajisuanhuanshoulv: volume=%s circulating=%s total_shares=%s",
                    symbol,
                    volume,
                    circulating,
                    total_shares,
        ...
        logger.info(
            f"[Longbridge] {symbol} hangqinghuoquchenggong: "
            f"jiage={price}, liangbi={volume_ratio}, huanshoulv={turnover_rate}"
        )
        return quote
