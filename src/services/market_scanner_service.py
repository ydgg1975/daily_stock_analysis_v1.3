...
class MarketScannerService:
    ...
    def run_scan(
        ...
        *,
        ...
        if profile_config.market != "cn":
            raise ValueError(f"当前阶段仅实现 A 股扫描，暂不支持市场: {profile_config.market}")
    ...
    @staticmethod
    def _default_quality_summary() -> Dict[str, Any]:
        return {
            "available": False,
            "review_window_days": DEFAULT_SCANNER_REVIEW_WINDOW_DAYS,
    ...
    def _build_benchmark_review(
        ...
        *,
        ...
        cache_key = (anchor_date.isoformat(), window_end_date.isoformat(), int(review_window_days))
    ...
    def _build_recent_quality_summary(self, runs: Sequence[MarketScannerRun]) -> Dict[str, Any]:
        summary = self._default_quality_summary()
        if not runs:
            return summary
    ...
    @staticmethod
    def _build_headline(shortlist: Sequence[Dict[str, Any]]) -> str:
        if not shortlist:
    ...
    @staticmethod
    def _build_scoring_notes() -> List[str]:
        return [
