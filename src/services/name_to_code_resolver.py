...
def resolve_name_to_code(name: str) -> Optional[str]:
    ...
    if akshare_map and s in akshare_map:
        logger.debug(f"[NameResolver] mingzhong AkShare yingshe: {s} -> {akshare_map[s]}")
    ...
    if len(s) > 2:
        ...
        if matches:
            logger.debug(f"[NameResolver] mingzhongmohupipei: input={s}, matched={matches[0]}")
        ...
        # such as "Samsung Electronics" -> "Samsung Electronics".
        typo_matches = difflib.get_close_matches(s, names, n=1, cutoff=0.7)
        if typo_matches and _is_single_char_typo(s, typo_matches[0]):
            logger.debug(f"[NameResolver] mingzhongdanziwuxiedoudi: input={s}, matched={typo_matches[0]}")
            return all_name_to_code[typo_matches[0]]

    logger.debug(f"[NameResolver] jiexishibai: {s}")
    return None
