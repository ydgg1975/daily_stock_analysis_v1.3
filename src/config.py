...
class Config:
    ...
    # === feishuyunwendangconfig ===

    feishu_app_id: Optional[str] = None

    feishu_app_secret: Optional[str] = None

    feishu_folder_token: Optional[str] = None  # mubiaowenjianjia Token
    ...
    gemini_model: str = "gemini-3.1-pro-preview"  # zhumodel
    ...
    openai_model: str = "gpt-5.5"  # OpenAI jianrongmodelmingcheng
    ...
    discord_main_channel_id: Optional[str] = None  # Discord zhupindao ID

    discord_webhook_url: Optional[str] = None  # Discord Webhook URL

    discord_interactions_public_key: Optional[str] = None  # Discord Interaction ruzhanyanqiangongyao
    ...
    slack_channel_id: Optional[str] = None  # Slack pindao ID (Bot moshibitian)



    # AstrBot notificationconfig
    ...
    serverchan3_sendkey: Optional[str] = None  # Serverjiang3 SendKey
    ...
    log_dir: str = "./logs"  # rizhiwenjianmulu

    log_level: str = "INFO"  # rizhijibie

    

    # === xitongconfig ===

    max_workers: int = 3  # dibingfafangfengjin

    debug: bool = False

    http_proxy: Optional[str] = None  # HTTP daili (liru: http://127.0.0.1:10809)

    https_proxy: Optional[str] = None # HTTPS daili
    ...
    # Discord jiqirenzhuangtai

    discord_bot_status: str = "Aguzhinenganalysis | /help"
    ...
    bot_enabled: bool = True              # shifouqiyongjiqirengongneng
    ...
    dingtalk_app_key: Optional[str] = None      # yingyong AppKey
    ...
    wecom_encoding_aes_key: Optional[str] = None    # xiaoxijiajiemimiyao

    wecom_agent_id: Optional[str] = None            # yingyong AgentId

    

    # Telegram jiqiren - yiyou telegram_bot_token, telegram_chat_id

    telegram_webhook_secret: Optional[str] = None   # Webhook miyao
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        # quebaohuanjingbianliangyijiazai
        ...
        if http_proxy:
            ...
            # guoneijinrongshujuyuanyumingliebiao

            domestic_domains = [

                'eastmoney.com',   # dongfangcaifu (Efinance/Akshare)

                'sina.com.cn',     # xinlangcaijing (Akshare)

                '163.com',         # wangyicaijing (Akshare)

                'tushare.pro',     # Tushare

                'baostock.com',    # Baostock

                'sse.com.cn',      # shangjiaosuo

                'szse.cn',         # shenjiaosuo

                'csindex.com.cn',  # zhongzhengzhishu

                'cninfo.com.cn',   # juchaozixun
            ...
            # shezhihuanjingbianliang (requests/urllib3/aiohttp duhuizunshoucishezhi)
        ...
        # qiweixiaoxileixingyuzuidazijieshuluoji
        ...
        return cls(
            ...
            email_sender_name=os.getenv('EMAIL_SENDER_NAME', 'daily_stock_analysisstockanalysiszhushou'),
            ...
            # Discord jiqirenkuozhanconfig

            discord_bot_status=os.getenv('DISCORD_BOT_STATUS', 'Aguzhinenganalysis | /help'),
    ...
    def _parse_market_review_region(cls, value: str) -> str:
        ...
        """Parse market review region and fall back to cn for invalid values."""
        import logging
        ...
        logging.getLogger(__name__).warning(
            ...
            f"Invalid MARKET_REVIEW_REGION value {value!r}; falling back to 'cn'. "
            "Allowed values: cn / hk / us / both."
        )
    ...
    def _parse_market_review_color_scheme(cls, value: str) -> str:
        ...
        logging.getLogger(__name__).warning(
            ...
            "Invalid MARKET_REVIEW_COLOR_SCHEME value %r; falling back to 'green_up'. "
            "Allowed values: green_up / red_up.",
            value,
    ...
    def refresh_stock_list(self) -> None:
        ...
        if env_path.exists():
            ...
            # zhijiecong .env wenjianduquzuixindeconfig
    ...
    def validate_structured(self) -> List[ConfigIssue]:
        ...
        if not self.stock_list:
            ...
            issues.append(ConfigIssue(
                ...
                message="weiconfigwatchlistguliebiao (STOCK_LIST)",
            ...
            if missing_group_stocks:
                ...
                issues.append(ConfigIssue(
                    ...
                    message=(
                        ...
                        "STOCK_GROUP_N is used only for notification routing and does not expand the analysis scope. "
        ...
        if not self.llm_model_list and not has_direct_env_model:
            ...
            issues.append(ConfigIssue(
                ...
                message=(
                    "Primary model is not explicitly configured; the system will infer it from available API keys. "
                    "Configure LITELLM_MODEL as soon as possible, for example gemini/gemini-3.1-pro-preview."
                ),
        ...
        if available_router_model_set:
            ...
            if (
                ...
                issues.append(ConfigIssue(
                    ...
                    message=(
                        "Configured primary model is not declared in the current channel or router configuration. "
            ...
            if (
                ...
                issues.append(ConfigIssue(
                    ...
                    message=(
                        "Configured Agent primary model is not declared in the current channel or router configuration. "
                        f"Available models: {', '.join(available_router_models[:6])}"
                    ),
            ...
            if invalid_fallbacks:
                ...
                issues.append(ConfigIssue(
                    ...
                    message=(
                        "Fallback models include entries not declared in the current channel or router configuration: "
            ...
            if (
                ...
                issues.append(ConfigIssue(
                    ...
                    message=(
                        "VISION_MODEL is not declared in the current channel configuration. "
                        f"Available models: {', '.join(available_router_models[:6])}"
                    ),
            ...
            issues.append(ConfigIssue(
                ...
                message=(
                    "Configured Agent primary model has no available runtime source. "
                    "Enable the matching channel or API key."
                ),
        ...
        if self.notification_quiet_hours:
            ...
            try:
                ...
                issues.append(ConfigIssue(
                    ...
                    message=f"Invalid notification quiet-hours configuration: {exc}",
        ...
        if self.notification_timezone:
            ...
            try:
                ...
                issues.append(ConfigIssue(
                    ...
                    message=f"Invalid notification timezone configuration: {exc}",
        ...
        if self.notification_daily_digest_enabled:
            ...
            issues.append(ConfigIssue(
                ...
                message=(
                    "NOTIFICATION_DAILY_DIGEST_ENABLED is currently reserved. "
                    "Daily digest notifications are not sent yet."
                ),
        ...
        if (
            ...
            issues.append(ConfigIssue(
                ...
                message=(
                    "FEISHU_APP_ID / FEISHU_APP_SECRET alone do not enable Feishu group Webhook notifications. "
                    "Set FEISHU_WEBHOOK_URL for group notifications, or enable FEISHU_STREAM_ENABLED and complete app permissions for the app bot."
                ),
        ...
        if os.getenv("OPENAI_VISION_MODEL"):
            ...
            issues.append(ConfigIssue(
                ...
                message=(
                    ...
                    "It is still migrated automatically for now, but update the configuration to remove this notice."
                ),
        ...
        if self.vision_model:
            ...
            if not _has_any_key:
                ...
                issues.append(ConfigIssue(
                    ...
                    message=(
                        "VISION_MODEL is configured, but no matching Vision API key was found. "
                        f"Checked providers: {', '.join(_checked)}. "
                        "Image-based stock extraction will be unavailable until the matching API key is configured."
                    ),
...
# === bianjiedeconfigfangwenhanshu ===

def get_config() -> Config:

    """huoququanjuconfigshilidekuaijiefangshi"""
...
if __name__ == "__main__":
    ...
    print("=== configjiazaitest ===")

    print(f"watchlistguliebiao: {config.stock_list}")
