...
```json
{
    ...
    "evidence_points": ["支持当前结论的关键数据 or 事实依据1", "依据2"],
    "counter_evidence": ["削弱或反驳当前结论的因素1", "因素2"],
    "data_limitations": ["数据缺口、时效性不足或推断边界1", "限制2"],
    "confidence_reason": "说明置信度高/中/低的原因",
    "analysis_confidence": {
...
- 不得仅因为单日涨跌 or 评分跨线就在“买入/卖出”之间剧烈切换。
- 操作建议必须同时参考价格位置（支撑/压力位）、量能/筹码、主力资金流向和风险事件。
- 股价位于支撑与压力之间、资金流不明确时，优先输出“持有/震荡/观望/洗盘观察”等可执行的中性建议；`decision_type` 仍保持 `hold`。
- 只有在接近支撑确认 or 有效突破压力，且资金流/量价配合时，才能给出买入；接近压力且资金流出时不得追买。
- 只有在跌破关键支撑、主力资金持续流出 or 风险显著放大时，才能给出卖出/减仓。"""
