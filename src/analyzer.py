...
**利好同理，分三档加分**：
- 重大利好（+15-25 分）：重大合同、并购、政策红利
- 中等利好（+10-15 分）：业绩超预期、机构增持
- 轻微利好（+5-10 分）：回购计划、获得专利

**确定性事件覆盖**（类似致命级利空的镜像规则）：
- 当消息面存在确定性极高的利好事件（已公告要约收购、溢价私有化等），
  技术面分析已失去意义，评分直接提升至 >= 80，此时第 1-7 条技术面规则不适用
- 判断标准：事件已正式公告且收购价/要约价明确高于现价

**多条新闻的合并规则**：
- 同一事件的多篇报道只计一次，不叠加
- 利好和利空可部分对冲，但致命级利空不可被对冲
- 确定性事件覆盖不可被普通利空对冲（与致命级利空同级时，致命级优先）
- 最终消息面调整幅度上限为 +/-30 分（确定性事件覆盖和致命级利空除外）

### 9. 新闻时效性权重

- 今日新闻：100% 权重
- 1-3 日前：70% 权重（影响分值打 7 折）
- 4-7 日前：40% 权重
- 7 日以上：仅作背景参考，不参与评分调整
- 无日期的新闻：按 50% 权重处理

**持续性事件例外**：以下事件的影响不随时间衰减，始终按 100% 权重计算：
- 大额解禁（解禁股份未被完全消化前持续有效）
- 立案调查 / 监管处罚（结论出来前持续有效）
- 行业政策变化（政策存续期间持续有效）
- 要约收购 / 重大资产重组（交易完成前持续有效）

示例：3 天前的"减持 2%"新闻，属于重大级（扣 15-25），
打 7 折后实际扣分 = 15 x 0.7 = 10 分。
但 6 天前的"大额解禁"属于持续性事件，不打折，仍按 100% 计算。
...
| 条件组合 | 基准分区间 |
|---------|-----------|
| 强势多头 + 乖离率 < 2% + 缩量回调 + 筹码健康 | 75-85 |
| 多头排列 + 乖离率 < 5% + 量能正常 | 60-75 |
| 强势趋势股（第 7 条）+ 乖离率 5-8% | 55-65 |
| 弱势多头 或 盘整 或 乖离率 > 5% | 45-60 |
| 弱势空头 | 30-45 |
| 空头排列 或 跌破 MA20 | 15-30 |
| 强势空头 + 放量下跌 | 0-15 |

基准分应参考算法系统评分（signal_score），两者偏差不应超过 15 分。
如有偏差需在 analysis_summary 中说明理由。

### 第二步：消息面调整分（-30 至 +30）

按第 8、9 条规则计算新闻影响力，得到调整分。

### 第三步：边界约束

- 最终分 = max(0, min(100, 基准分 + 调整分))
- 乖离率硬上限：乖离率 > 5%（强势趋势股 > 8%）时，最终分不得超过 59（观望上限）
- 致命级利空直接覆盖：无论技术面多好，评分 <= 39
- 确定性利好直接覆盖：确定性极高的利好事件（已公告收购且溢价明确），评分 >= 80
- 覆盖规则优先级：致命级利空 > 确定性利好 > 乖离率硬上限 > 正常公式
- 无新闻时调整分 = 0，直接使用技术面基准分
