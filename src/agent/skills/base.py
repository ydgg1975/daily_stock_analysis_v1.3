...
(e.g., longtoucelve, suolianghuicai, junxianjincha) used for analysis and push notifications.
...
class Skill:
    ...
    Attributes:
        ...
        display_name: Human-readable name (e.g., "longtoucelve").
        ...
        category: Skill category — "trend" (qushi), "pattern" (xingtai), "reversal" (fanzhuan), "framework" (kuangjia).
...
class SkillManager:
    ...
    def get_skill_instructions(self) -> str:
        ...
        # Group by category
        categories = {"trend": "qushi", "pattern": "xingtai", "reversal": "fanzhuan", "framework": "kuangjia"}
        ...
        for cat_key in ordered_keys + [k for k in grouped if k not in ordered_keys]:
            ...
            parts.append(f"#### {cat_label}leijineng\n")
            for skill in skills_in_cat:
                rules_ref = ""
                if skill.core_rules:
                    rules_ref = f"（guanlianhexinlinian：di{'、'.join(str(r) for r in skill.core_rules)}tiao）"
                support_ref = ""
                if skill.bundle_dir and skill.entrypoint.endswith("SKILL.md"):
                    support_ref = "（bundle）"
                parts.append(
                    f"### jineng {idx}: {skill.display_name} {rules_ref}{support_ref}\n\n"
                    f"**shiyongchangjing**: {skill.description}\n\n"
