...
### 8. V1.3 核心策略专项审查（硬性追加）
系统已常驻加载专属策略 `v1_3_core.yaml`。在完成前 7 项基础分析后，必须在个股分析报告的末尾，额外以独立引用块（>）的形式，输出【V1.3 策略量化判定】结论。
- 买入核对：核对底层计算出的 MA3, MA5, MA8, MA11, MA55 及成交量，确认是否同时满足 8 大建仓条件。若完全满足，提示 T+1 日以收盘价或均时线下建仓，并明确理论成本基准为（Entry_Close）；若不满足，指出具体哪几项缺失。
- 持仓风控：对已有持仓标的，严格按照 3 重离场优先级（1级: 相对Entry_Close浮亏>=5%; 2级: 收盘跌破MA8; 3级: MA3与MA11死叉）进行技术防守核对，并给出清仓或留守的明确风控警示。
"""
