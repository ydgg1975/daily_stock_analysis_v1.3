...
def _handle_calculate_ma(stock_code: str, periods: Optional[str] = None, days: int = 120) -> dict:
    ...
    result["ma_alignment"] = (
        ...
        "duotoupailie" if above_count == len(ma_values)

        else "kongtoupailie" if above_count == 0

        else f"hunhe({above_count}/{len(ma_values)}tiaojunxianshangfang)"
...
calculate_ma_tool = ToolDefinition(
    ...
    description="Calculate moving averages (MA5/10/20/30/60/120/250 or custom periods) "
                ...
                "is above each MA. Also returns overall MA alignment (duotou/kongtou/hunhe).",
...
def _handle_get_volume_analysis(stock_code: str, days: int = 30) -> dict:
    ...
    if len(volume) >= 10:
        ...
        vol_trend = "fangliang" if vol_trend_pct > 20 else "suoliang" if vol_trend_pct < -20 else "liangnengpingwen"
    ...
    if avg_up_vol > avg_down_vol * 1.3:
        ...
        pattern = "거래량과 가격 흐름이 양호합니다. 상승 시 거래량 증가, 하락 시 거래량 감소 흐름입니다."
    elif avg_down_vol > avg_up_vol * 1.3:

        pattern = "거래량과 가격 흐름이 좋지 않습니다. 하락 시 거래량 증가, 상승 시 거래량 감소 흐름입니다."
    elif vol_ratio_5d and vol_ratio_5d > 1.5:

        pattern = "jinqimingxianfangliang"

    elif vol_ratio_5d and vol_ratio_5d < 0.6:

        pattern = "jinqimingxiansuoliang"

    else:

        pattern = "liangjiaguanxineutral"
...
get_volume_analysis_tool = ToolDefinition(
    ...
    description="Analyse volume-price relationship for a stock. Returns volume ratios, "
                ...
                "and pattern interpretation (liangjiapeihe/beili). Useful for confirming trend "
...
def _handle_analyze_pattern(stock_code: str, days: int = 60) -> dict:
    ...
    for i in range(max(0, n - 3), n):
        ...
        if bd < avg_body * 0.1 and (us + ls) > bd * 3:
            ...
            patterns_detected.append({
                ...
                "pattern": "shizixing (Doji)", "type": "reversal_signal",
        ...
        if ls > body(i) * 2 and us < body(i) * 0.5:
            ...
            label = "chuizixian (Hammer)" if i == 0 or c[i] >= c[i - 1] else "shangdiaoxian (Hanging Man)"
        ...
        if us > body(i) * 2 and ls < body(i) * 0.5:
            ...
            label = "liuxingxian (Shooting Star)" if is_bearish(i) else "daochuizi"
        ...
        if bd > avg_body * 2.5:
            ...
            label = "dayangxian" if is_bullish(i) else "dayinxian"
    ...
    if n >= 3:
        ...
        # Morning Star (zaochenzhixing) ??bottom reversal
        ...
        if (is_bearish(i - 2) and body(i - 2) > avg_body * 1.5
            ...
            patterns_detected.append({
                ...
                "pattern": "zaochenzhixing (Morning Star)", "type": "bullish_reversal",

                "day_offset": -2, "strength": "qiang", "desc": "sangenKxiandibufanzhuanxingtai"

            })



        # Evening Star (huanghunzhixing) ??top reversal
        ...
        if (is_bullish(i - 2) and body(i - 2) > avg_body * 1.5
            ...
            patterns_detected.append({
                ...
                "pattern": "huanghunzhixing (Evening Star)", "type": "bearish_reversal",

                "day_offset": -2, "strength": "qiang", "desc": "sangenKxiandingbufanzhuanxingtai"

            })



        # Engulfing (tunmodeltai)
        ...
        if (is_bullish(i) and is_bearish(i - 1)
            ...
            patterns_detected.append({
                ...
                "pattern": "kanzhangtunmo (Bullish Engulfing)", "type": "bullish_reversal",

                "day_offset": -1, "strength": "qiang", "desc": "yangxianwanquanfugaiqianyiyinxian"
            ...
            patterns_detected.append({
                ...
                "pattern": "kandietunmo (Bearish Engulfing)", "type": "bearish_reversal",

                "day_offset": -1, "strength": "qiang", "desc": "yinxianwanquanfugaiqianyiyangxian"
    ...
    # Double bottom detection (jianhuaban: lianggexiangjindidian + zhongjiangaodian)
        ...
        if lo2 - lo1 >= 5 and abs(l[lo1] - l[lo2]) / max(l[lo1], l[lo2]) < 0.03:
            ...
            if mid_high > l[lo1] * 1.03:
                ...
                patterns_detected.append({
                    ...
                    "pattern": "shuangdi (Double Bottom)", "type": "bullish_reversal",
    ...
    if n >= 10:
        ...
        if box_range_pct < 8:
            ...
            patterns_detected.append({
                ...
                "pattern": "xiangtizhendang", "type": "consolidation",
    ...
    return {
        ...
        "summary": (
            ...
            "뚜렷한 패턴을 발견하지 못했습니다." if not unique_patterns
