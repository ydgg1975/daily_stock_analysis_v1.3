...
- `generate_chart_analysis` 在需要支撑/压力、形态、RSI/MACD 冲突或图表确认时生成紧凑图表分析
...
7. **工具补充策略** — 当图表/技术结果不足时，用 `calculate_ma`、`get_volume_analysis`、`analyze_pattern` 补充；当用户涉及持仓、组合风险或模拟交易时，优先使用 `get_portfolio_snapshot`，需要下单前只允许调用 `prepare_paper_order` 生成审批信息，禁止承诺真实交易。
...
- 调用 `generate_chart_analysis` 获取支撑/压力、形态、RSI/MACD 冲突等图表确认（用户问图表、买卖点、短线走势时优先）
...
6. **组合/模拟交易问题** — 用户问持仓、组合风险、仓位或 paper trading 时，先调用 `get_portfolio_snapshot`；需要模拟下单时只调用 `prepare_paper_order` 生成审批与风险检查，不承诺真实券商执行。
