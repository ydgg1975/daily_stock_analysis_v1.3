...
def apply_phase_decision_guardrails(
    ...
    *,
    ...
    if (
        core_degraded
        and _is_high_confidence(getattr(result, "confidence_level", ""))
        and _has_buy_sell_signal(result, phase_decision, language=language)
    ):
        result.confidence_level = "Medium" if language == "en" else "中"
        reason = (
            "Core quote, daily-bar, or technical data is degraded; high-confidence buy/sell was capped."
            if language == "en"
            else "核心行情、日线或技术数据受限，已限制高置信买卖结论。"
        )
...
def _has_buy_sell_signal(
    result: "AnalysisResult",
    phase_decision: Mapping[str, Any],
    *,
    language: str,
) -> bool:
    decision_type = _safe_text(getattr(result, "decision_type", "")).lower()
    ...
    markers = _BUY_SELL_MARKERS_EN if language == "en" else _BUY_SELL_MARKERS_ZH
    return _contains_non_negated_marker(haystack, markers, language=language)
