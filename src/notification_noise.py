    ...
    if min_severity_raw:
        ...
        if min_severity_raw not in NOTIFICATION_SEVERITY_RANK:
            ...
            return NotificationNoiseDecision(
                ...
                message=(
                    f"Notification severity {resolved_severity} is below minimum severity {min_severity_raw}; "
                    "static notification channels were skipped."
                ),
    ...
    with _state_lock:
        ...
        if dedup_ttl > 0 and _dedup_expires_at.get(dedup_state_key, 0) > now_ts:
            ...
            return NotificationNoiseDecision(
                ...
                message="Notification content was already sent within the deduplication TTL; static channels were skipped.",
        ...
        if dedup_ttl > 0 and dedup_inflight and dedup_inflight[0] > now_ts:
            ...
            return NotificationNoiseDecision(
                ...
                message="An identical notification is already being sent; static channels were skipped.",
        ...
        if cooldown > 0 and _cooldown_expires_at.get(cooldown_state_key, 0) > now_ts:
            ...
            return NotificationNoiseDecision(
                ...
                message="Notification cooldown has not ended; static channels were skipped.",
