...
def _build_ci_context():
    ...
    if not auto_check_result:
        return """
## CI jianchazhuangtai
> ⚠️ weihuoqudao CI jianchajieguo。shenchashibudejiashe CI yitongguo，yanzhengxiangguanpanduanyingbiaozhuwei"wufaqueren"。
"""
...
def build_prompt(diff_content, files, truncated, pr_title, pr_body):
    ...
    return f"""nishibencangkude PR shenchazhushou。qinggenjubiangengneironghe PR miaoshu，zhixing“daima + wendang + CI”lianheshencha。

## PR xinxi
- biaoti: {pr_title or '(empty)'}
- miaoshu:
{pr_body or '(empty)'}
...
```
{ci_context}
## bixuduiqideshenchaguize（laizicangku AGENTS.md）
1. biyaoxing（Necessity）：shifouyoumingquewenti/yewujiazhi，bimianwuxiaozhonggou。
2. guanlianxing（Traceability）：shifouyouguanlian Issue（Fixes/Refs）；ziranyuyanguanlian（ru"guanlian issue wei #xxx"）yekejieshou，buyingeshiwentipandingbutongguo。wu Issue shishifougeichudongjiyuyanshoubiaozhun。
3. leixingpanding（Type）：fix/feat/refactor/docs/chore/test shifoupipei。
4. miaoshuwanzhengxing（Description Completeness）：shifoubaohanbeijing、fanwei、yanzhengminglingyujieguo、jianrongxingfengxian、huigunfangan。panduanyanzhengshifouchongfenshi，bixucankaoshangfang"CI jianchazhuangtai"duanluo：（a）ruo py_compile he flake8 yitongguo，PR miaoshuzhongkeyinyong CI jieguoerbubitieduiyingbendishuchu；（b）`./scripts/ci_gate.sh` buzai CI fugaifanwei，dui Python houduangaidongxujiancha PR miaoshushifoushuominglegai gate dezhixingqingkuang，ruoweishuomingyinglieweijianyixiang；（c）ruoweitigong CI jieguo，zebudejiashe CI yitongguo，yanzhengchongfenxingyingbiaozhuwei"wufaqueren"。
5. herupanding（Merge Readiness）：geichu Ready / Not Ready，bingliechuzuduanxiang。
6. ruoshejiyonghukejiannengli，jiancha README.md yu docs/CHANGELOG.md shifoutongbu。

## zuduan vs jianyidepandingbiaozhun
jinyixiawentikepandingwei Not Ready（zuduanxiang/bigaixiang）：
- daimacunzaizhengquexinghuoanquanxingwenti（luojicuowu、yichangtunmo、anquanloudongdeng）
- CI jianchaweitongguo
- PR miaoshuyushijigaidongneirongcunzaishizhixingmaodun
- queshaohuigunfangan

yixiawentijinfangrujianyixiang，buyingxiangherupanding：
- issue guanliangeshibuguifan
- yufa/flake8 yanzhengzhengjuqueshidanshangfang"CI jianchazhuangtai"xianshi py_compile he flake8 juntongguo
- Python houduangaidongde PR miaoshuweishuoming `./scripts/ci_gate.sh` shifouzhixinghuogeichutiaoguoyuanyin
- miaoshuzhongfeiguanjianxingcuocihuogeshiwenti
- zhushiyuyanfengge、wuguansuowenjianbiangengdeng

## shenchashuchuyaoqiu
- shiyongzhongwen。
- xiangei"jielun"：`Ready to Merge` huo `Not Ready`。
- zaigeijiegouhuajieguo：
  - biyaoxing：tongguo/butongguo + liyou
  - guanlianxing：tongguo/butongguo + zhengju
  - leixing：jianyileixing
  - miaoshuwanzhengxing：wanzheng/buwanzheng（queshixiang）
  - fengxianjibie：di/zhong/gao + guanjianfengxian
  - bigaixiang（zuiduo 5 tiao，jinxianzuduantiaojian，anyouxianji）
  - jianyixiang（zuiduo 5 tiao）
- bigaixiangjinbaohanshangshuzuduantiaojianzhongdewenti；geshi、guanlian、yanzhengzhengjudengfeizuduanwentifangrujianyixiang。
- duifaxiandewenti，jinliangdingweidaowenjianlujingbingshuomingyingxiang。
- ruguoxinxibuzu，mingquexie“jiyudangqian diff/PR miaoshuwufaqueren”。
"""
...
def review_with_openai(prompt):
    ...
    try:
        ...
        print(f"✅ OpenAI jianrongjiekou ({model}) shenchachenggong")
        ...
        print(f"❌ OpenAI jianrongjiekoushenchashibai: {e}")
        traceback.print_exc()
        return None
...
def ai_review(diff_content, files, truncated):
    ...
    print("changshishiyong OpenAI jianrongjiekou...")
...
def main():
    ...
    if review:
        ...
        print("⚠️ suoyou AI jiekoudoubukeyong")
        if summary_file:
            with open(summary_file, 'a', encoding='utf-8') as f:
                f.write("## 🤖 AI daimashencha\n\n⚠️ AI jiekoubukeyong，qingjianchapeizhi\n")
        if strict_mode:
