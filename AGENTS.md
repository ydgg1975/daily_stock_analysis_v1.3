...
- 默认采用短输出模式，避免接近 32k 输出上限：先给结论、Top-N 重点、关键证据、文件路径与验证结果。
- 除非用户明确要求全文，禁止一次性输出完整 log、完整 diff、大型 JSON/CSV、全文文件、多 agent 原文汇总或逐行长篇分析；完整内容优先保存到文件并回报路径，或按用户指定分段展开。
- 大型审查、研究、排障、报告或多 agent 汇总时，子任务只返回“结论 / 证据 / 风险 / 路径”，主回复只做去重摘要；若结果仍过长，改为索引式回复并等待用户指定展开。
