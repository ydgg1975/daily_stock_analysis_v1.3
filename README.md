...
系统内置多链加密货币新币扫描器，支持自动发现、风险分析、AI 研判与告警推送。
...
| 阶段    | 功能                                   | 配置入口                          |
| ------- | -------------------------------------- | --------------------------------- |
| Phase 1 | 多链新币发现与快照追踪                 | `CRYPTO_ENABLED`, `CRYPTO_CHAINS` |
| Phase 2 | 风险评估 + 观察列表 + 阈值告警         | `CRYPTO_ALERTS_ENABLED`           |
| Phase 3 | AI 多分析师管线（市场/安全/社交/技术） | `CRYPTO_AI_ENRICHMENT_ENABLED`    |
| Phase 4 | 可观测性仪表盘（SLO/成本/Prompt 对比） | Web UI                            |

**API 端点：** `/api/v1/crypto/launches`, `/api/v1/crypto/status`, `/api/v1/crypto/refresh`

> 详细配置与架构说明见 [docs/crypto-scanner.md](docs/crypto-scanner.md)
