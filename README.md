...
> Web 报告页现已把顶部价格口径收敛为更直观的 `Analysis Price / Price basis / Session / Updated / Report time`，盘中场景明确标记为 `Intraday snapshot` 而不是伪装成实时流；报告首屏同时拆成真正的 desktop/mobile 双模式：桌面端用宽 Hero + 右侧状态栏吃满横向空间，手机端则把次级元信息折叠、让图表更早出现。主摘要下方同步接入 API 驱动的金融图表，支持 `1D / 1M / 3M / 1Y / W / M` 视图、K 线 + 成交量 + MA 叠加与关键价位标线，并在分析完成后自动打开最新报告，仅以短时 toast 提示成功，不再长期占用页面空间。
...
最近一轮 Web 工作台重点补齐了四类体验：分析完成后会自动打开同股票的最新报告并同步高亮历史记录；首页、报告、持仓、回测统一使用同一套宽度/间距规则；手机端恢复正常纵向滚动，报告区会按单列优先展开；`赛博朋克 / Geek / DOS` 主题不再只是换底色，而会同步影响卡片、按钮、输入框、圆角、字体与界面氛围。
