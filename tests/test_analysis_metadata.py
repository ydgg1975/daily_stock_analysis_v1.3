...
class TestSelectionSourcePatternEdgeCases:
    ...
    def test_pattern_unicode(self):
        ...
        assert pattern.fullmatch("shoudongshuru") is None
        assert pattern.fullmatch("zidongbuquan") is None
        assert pattern.fullmatch("tupianshibie") is None

        # Mixed characters should fail
        assert pattern.fullmatch("manualshuru") is None
        assert pattern.fullmatch("autocompleteshibie") is None
    ...
    def test_source_use_cases(self):
        ...
        use_cases = {
            "manual": [
                ...
                "User directly inputs guizhoumaotai in input box",
            ],
            "autocomplete": [
                "User inputs 'maotai', selects 'guizhoumaotai' from dropdown",
                "User inputs 'gzmt', selects 'guizhoumaotai' from dropdown",
