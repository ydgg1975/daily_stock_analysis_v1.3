...
class TestSkillManager(unittest.TestCase):
    ...
    def test_get_skill_instructions_content(self):
        ...
        self.assertIn("jineng 1:", instructions)
...
display_name: ceshiYAMLcelve
description: yigeyongyuceshidecelve
category: trend
core_rules: [1, 3]
required_tools:
  - analyze_trend
...
instructions: |
  **ceshicelve**
  ...
  zheshiyigeyongziranyuyanbianxiedeceshicelve。
  panduanbiaozhun：dang MA5 > MA10 shimairu。
"""
        ...
        try:
            ...
            self.assertEqual(skill.display_name, "ceshiYAMLcelve")
            ...
            self.assertIn("ziranyuyan", skill.instructions)
    ...
    def test_load_minimal_yaml(self):
        ...
        yaml_content = """
name: minimal
display_name: zuijiancelve
description: zuijianmiaoshu
instructions: yongziranyuyanmiaoshudecelveneirong
"""
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False, encoding='utf-8') as f:
            f.write(yaml_content)
            tmp_path = f.name
    ...
    def test_load_yaml_metadata_fields(self):
        ...
        yaml_content = """
name: metadata_skill
display_name: yuanshujujineng
description: daiyoumorenyuanshujudejineng
aliases: [biemingyi, bieminger]
default_active: true
default_router: true
default_priority: 15
market_regimes: [trending_up, volatile]
instructions: |
  zheshiyigeceshijineng。
"""
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False, encoding='utf-8') as f:
            f.write(yaml_content)
            tmp_path = f.name
        ...
        try:
            skill = load_skill_from_yaml(tmp_path)
            self.assertEqual(skill.aliases, ["biemingyi", "bieminger"])
        ...
        try:
            ...
            for i, name in enumerate(["strategy_a", "strategy_b"]):
                with open(os.path.join(tmpdir, f"{name}.yaml"), 'w', encoding='utf-8') as f:
                    f.write(f"""
name: {name}
display_name: celve{chr(65 + i)}
description: miaoshu{chr(65 + i)}
instructions: ziranyuyancelvemiaoshu {name}
""")
...
aliases: [lundong, longtouzhencha]
default-active: true
default-router: true
default-priority: 12
...
""",
            ...
            skill = load_skill_from_markdown(skill_dir / "SKILL.md")
            self.assertEqual(skill.aliases, ["lundong", "longtouzhencha"])
            self.assertTrue(skill.default_active)
            self.assertTrue(skill.default_router)
    ...
    def test_custom_overrides_builtin(self):
        ...
        try:
            with open(os.path.join(tmpdir, "dragon_head.yaml"), 'w', encoding='utf-8') as f:
                f.write("""
name: dragon_head
display_name: zidingyilongtoucelve
description: wozijidelongtoucelve
instructions: anzhaowodeguizefenxilongtougu
""")
            manager.load_custom_strategies(tmpdir)

            overridden = manager.get("dragon_head")
            self.assertEqual(overridden.display_name, "zidingyilongtoucelve")
