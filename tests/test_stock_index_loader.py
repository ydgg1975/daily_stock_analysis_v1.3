    ...
    def test_get_index_stock_name_supports_display_canonical_and_hk_keys(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            index_path.write_text(
                json.dumps(
                    [
                        ["000001.SZ", "000001", "pinganyinhang", "pinganyinhang", "payh", [], "CN", "stock", True, 100],
                        ["00700.HK", "00700", "tengxunkonggu", "tengxunkonggu", "txkg", [], "HK", "stock", True, 100],
                        ["AAPL", "AAPL", "pingguo", "pingguo", "pg", [], "US", "stock", True, 100],
                    ],
                    ensure_ascii=False,
                ),
                encoding="utf-8",
            ...
            with patch.object(stock_index_loader, "get_stock_index_candidate_paths", return_value=(index_path,)):
                self.assertEqual(stock_index_loader.get_index_stock_name("000001"), "pinganyinhang")
                self.assertEqual(stock_index_loader.get_index_stock_name("000001.SZ"), "pinganyinhang")
                self.assertEqual(stock_index_loader.get_index_stock_name("HK00700"), "tengxunkonggu")
                self.assertEqual(stock_index_loader.get_index_stock_name("00700"), "tengxunkonggu")
                self.assertEqual(stock_index_loader.get_index_stock_name("700.HK"), "tengxunkonggu")
                self.assertEqual(stock_index_loader.get_index_stock_name("aapl"), "pingguo")

    def test_get_stock_name_index_map_is_cached_after_first_load(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            index_path = Path(temp_dir) / "stocks.index.json"
            index_path.write_text(
                json.dumps([["000001.SZ", "000001", "pinganyinhang"]], ensure_ascii=False),
                encoding="utf-8",
            )
            ...
            with patch.object(stock_index_loader, "get_stock_index_candidate_paths", return_value=(index_path,)):
                ...
                index_path.write_text(
                    json.dumps([["000001.SZ", "000001", "biangenghoumingcheng"]], ensure_ascii=False),
                    encoding="utf-8",
                )
    ...
    def test_get_stock_name_index_map_skips_unexpected_json_shape_and_uses_next_candidate(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            malformed_path.write_text(
                json.dumps({"code": "000001", "name": "pinganyinhang"}, ensure_ascii=False),
                encoding="utf-8",
            )
            valid_path.write_text(
                json.dumps([["000001.SZ", "000001", "pinganyinhang"]], ensure_ascii=False),
