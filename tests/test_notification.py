...
class TestNotificationServiceReportGeneration(unittest.TestCase):
    ...
    def test_notification_renderer_to_discord_keeps_full_sections_after_chunking(
        ...
        self.assertIn("当前价: 125.40", merged)
        self.assertIn("涨跌幅: 1.20%", merged)
        self.assertIn("最高: 126.80", merged)
        self.assertIn("最低: 123.90", merged)
        self.assertIn("成交量: 3456万", merged)
        self.assertIn("MA5: 124.80", merged)
        self.assertIn("MA10: 123.70", merged)
        self.assertIn("MA20: 122.10", merged)
        self.assertIn("支撑位: 121.50", merged)
        self.assertIn("压力位: 127.00", merged)
        self.assertIn("理想买入点: 123.00-124.00", merged)
        self.assertIn("止损位: 119.00", merged)
        self.assertIn("目标位: 132.00", merged)
        self.assertIn("建议仓位: 30.00%", merged)
        self.assertIn("执行确认：重点确认买点、量价配合和止损纪律。", merged)
        self.assertNotIn("|字段|数值|", merged)
        self.assertNotIn("market_timestamp", merged)
        self.assertNotIn("report_generated_at", merged)
        self.assertNotIn("news_published_at", merged)
        self.assertIn("基本面：", merged)
        self.assertIn("关键指标：", merged)
        self.assertIn("财报趋势：", merged)
        self.assertIn("情绪：", merged)
        self.assertNotIn("### 🧾 基本面摘要（Fundamentals）", merged)
        self.assertNotIn("### 🧩 数据质量说明", merged)
        self.assertNotIn("qoq_revenue_growth", merged)
        self.assertNotIn("medium", merged)
        self.assertNotIn("company_sentiment", merged)
        self.assertNotIn("overall_confidence", merged)
