...
Aguzixuanguzhinengfenxixitong - tongzhifuwudanyuanceshi
...
1. yanzhengtongzhifuwudepeizhijianceluoji

2. yanzhengtongzhifuwudequdaojianceluoji

3. yanzhengtongzhifuwudexiaoxifasongluoji



TODO: 

1. tianjiafasongqudaoyiwaideceshi，ru：

    - shengchengribao

2. tianjia send_to_context deceshi
...
class TestNotificationServiceSendToMethods(unittest.TestCase):
    ...
    ceshianzhaoqudaodezimushunxupailie，zaiheshiweizhitianjiaxindeceshifangfa。

    ruguocaiyongzhangxiaoxifenpifasong，bixudanduceshifenpifasongdeluoji，
    ...
    shiyong mock.patch zhuangshiqilaimoni get_config hanshu，

    shiyong _make_config hanshutianjiapeizhi，bingfanhui Config shili。



    2. jianchapeizhishifouzhengque：

    shiyong assertIn jiancha NotificationChannel.xxxx shifouzai

    `NotificationService.get_available_channels()` fanhuizhizhong。



    3. moniqingqiuxiangying：

    shiyong mock.patch zhuangshiqilaimoni requests.post hanshu，

    shiyong _make_response hanshumoniqingqiuxiangying，bingfanhui Response shili。

    ruoshiyongqitahanshumoniqingqiuxiangying，zeshiyong mock.patch zhuangshiqilaimonigaihanshu。



    4. shiyong assertTrue jiancha send defanhuizhi。



    5. shiyong assert_called_once jianchaqingqiuhanshushifoubeidiaoyongyici。

    ceshifenpifasongshi，shiyong assertAlmostEqual(mock_post.call_count, ...) jianchaqingqiuhanshubeidiaoyongcishu
...
class TestNotificationServiceReportGeneration(unittest.TestCase):
    ...
    """baogaoshengchengyuxuanluxiangguanceshi。"""
    ...
    def test_generate_reports_hide_model_when_disabled(self, mock_get_config: mock.MagicMock):
        ...
        self.assertNotIn("fenximoxing", dashboard)

        self.assertNotIn("gemini/gemini-2.5-flash", dashboard)

        self.assertNotIn("fenximoxing", single)
    ...
    def test_generate_dashboard_report_localizes_english_no_dashboard_fallback(
        ...
        self.assertNotIn("caozuoliyou", out)

        self.assertNotIn("fengxiantishi", out)

        self.assertNotIn("jishumian", out)

        self.assertNotIn("xiaoximian", out)
