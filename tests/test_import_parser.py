...
class TestParseImportFromBytesCsv:
    def test_parses_csv_with_header(self):
        data = "code,name\n600519,guizhoumaotai\n00700,tengxunkonggu".encode("utf-8")
        result = parse_import_from_bytes(data, "a.csv")
        assert len(result) == 2
        assert result[0] == ("600519", "guizhoumaotai", "medium")
        assert result[1] == ("00700", "tengxunkonggu", "medium")

    def test_parses_csv_chinese_column_names(self):
        data = "gupiaodaima,gupiaomingcheng\n600519,guizhoumaotai".encode("utf-8")
        result = parse_import_from_bytes(data, "a.csv")
        assert result[0] == ("600519", "guizhoumaotai", "medium")

    def test_parses_csv_no_header(self):
        # Use 300750 instead of 00700 to avoid pandas stripping leading zeros
        data = "600519,guizhoumaotai\n300750,ningdeshidai".encode("utf-8")
        result = parse_import_from_bytes(data, "a.csv")
        assert len(result) == 2
        assert result[0] == ("600519", "guizhoumaotai", "medium")
        assert result[1] == ("300750", "ningdeshidai", "medium")
    ...
    def test_resolves_name_when_code_empty(self, mock_resolve):
        ...
        data = "code,name\n,guizhoumaotai".encode("utf-8")
        result = parse_import_from_bytes(data, "a.csv")
        assert result[0] == ("600519", "guizhoumaotai", "medium")
        mock_resolve.assert_called_with("guizhoumaotai")

    @patch("src.services.import_parser.resolve_name_to_code")
    def test_returns_none_code_when_resolution_fails(self, mock_resolve):
        mock_resolve.return_value = None
        data = "code,name\n,bucunzaidegupiao".encode("utf-8")
        result = parse_import_from_bytes(data, "a.csv")
        assert result[0] == (None, "bucunzaidegupiao", "medium")
...
class TestParseImportFromBytesExcel:
    def test_parses_xlsx(self):
        ...
        ws = wb.active
        ws.append(["code", "name"])
        ws.append(["600519", "guizhoumaotai"])
        ws.append(["300750", "ningdeshidai"])
        buf = io.BytesIO()
        wb.save(buf)
        buf.seek(0)
        data = buf.read()
    ...
    def test_rejects_xls(self):
        data = b"dummy"
        with pytest.raises(ValueError, match="jinzhichi .xlsx"):
    ...
    def test_excel_error_includes_actionable_hint(self):
        ...
        assert "qingqueren" in msg or ".xlsx" in msg
...
class TestParseImportLimits:
    def test_rejects_file_over_limit(self):
        ...
        with pytest.raises(ValueError, match="chaoguo"):
    ...
    def test_csv_parser_error_raises_helpful_message(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            parse_import_from_bytes(data, "a.csv")
        msg = str(exc_info.value)
        assert "CSV jiexishibai" in msg
        assert "fengefu" in msg or "yinhao" in msg

    def test_accepts_gbk_encoded_csv(self):
        # Build CSV with Chinese in GBK encoding
        data = ("code,name\n600519," + "guizhoumaotai").encode("gbk")
        result = parse_import_from_bytes(data, "a.csv")
        assert len(result) == 1
        assert result[0][0] == "600519"
        assert result[0][1] == "guizhoumaotai"
...
class TestParseImportFromText:
    ...
    def test_preserves_name_when_code_is_dirty(self):
        data = "code,name\nINVALID,guizhoumaotai".encode("utf-8")
        result = parse_import_from_bytes(data, "a.csv")
        assert len(result) == 1
        assert result[0] == ("600519", "guizhoumaotai", "medium")
