...
class StockAnalyzerBiasTestCase(unittest.TestCase):
    ...
    def test_bias_negative_pullback(self, mock_get_config: MagicMock) -> None:
        """bias=-2% should yield 'huicaimaidian'."""
        mock_get_config.return_value.bias_threshold = 5.0
        result = _make_result(
            trend_status=TrendStatus.BULL,
            bias_ma5=-2.0,
        ...
        self.analyzer._generate_signal(result)
        self._assert_contains(result.signal_reasons, "huicaimaidian")

    @patch("src.stock_analyzer.get_config")
    def test_bias_close_to_ma5(self, mock_get_config: MagicMock) -> None:
        """bias=1.5% should yield 'jieruhaoshiji'."""
        mock_get_config.return_value.bias_threshold = 5.0
        result = _make_result(
            trend_status=TrendStatus.BULL,
            bias_ma5=1.5,
        ...
        self.analyzer._generate_signal(result)
        self._assert_contains(result.signal_reasons, "jieruhaoshiji")

    @patch("src.stock_analyzer.get_config")
    def test_bias_slightly_high(self, mock_get_config: MagicMock) -> None:
        """bias=4% (< base_threshold=5%) should yield 'kexiaocangjieru'."""
        mock_get_config.return_value.bias_threshold = 5.0
        result = _make_result(
            trend_status=TrendStatus.BULL,
            bias_ma5=4.0,
        ...
        self.analyzer._generate_signal(result)
        self._assert_contains(result.signal_reasons, "kexiaocangjieru")

    @patch("src.stock_analyzer.get_config")
    def test_strong_trend_relaxed_threshold(self, mock_get_config: MagicMock) -> None:
        """STRONG_BULL + trend_strength=75 + bias=6% -> 'keqingcangzhuizong' (effective=7.5%)."""
        mock_get_config.return_value.bias_threshold = 5.0
        result = _make_result(
        ...
        self.analyzer._generate_signal(result)
        self._assert_contains(result.signal_reasons, "keqingcangzhuizong")
        self._assert_not_contains(result.risk_factors, "yanjinzhuigao")

    @patch("src.stock_analyzer.get_config")
    def test_non_strong_trend_strict_threshold(self, mock_get_config: MagicMock) -> None:
        """BULL + bias=6% -> 'yanjinzhuigao!'."""
        mock_get_config.return_value.bias_threshold = 5.0
        result = _make_result(
            trend_status=TrendStatus.BULL,
            bias_ma5=6.0,
        ...
        self.analyzer._generate_signal(result)
        self._assert_contains(result.risk_factors, "yanjinzhuigao")

    @patch("src.stock_analyzer.get_config")
    def test_strong_trend_exceed_effective(self, mock_get_config: MagicMock) -> None:
        """STRONG_BULL + trend_strength=80 + bias=10% -> 'yanjinzhuigao!' (exceeds 7.5%)."""
        mock_get_config.return_value.bias_threshold = 5.0
        result = _make_result(
            trend_status=TrendStatus.STRONG_BULL,
            trend_strength=80.0,
    ...
    def test_boundary_at_base_threshold(self, mock_get_config: MagicMock) -> None:
        """bias=5.0% (exact base_threshold) -> 'kexiaocangjieru' (bias < base_threshold is False)."""
        mock_get_config.return_value.bias_threshold = 5.0
        result = _make_result(
            trend_status=TrendStatus.BULL,
            bias_ma5=5.0,
        ...
        # else: 5 > 5 False, so we'd get to the else branch with "yanjinzhuigao"
        ...
        # else: risks.append yanjinzhuigao - so we get yanjinzhuigao
