...
def _current_timestamp() -> str:
    """fanhuidangqian Unix miaozifuchuan，yongyushengchengyouxiaoqianming。"""
...
def test_signed_interaction_request_returns_deferred_ack():
    """type=2 jiaohuyingfanhui type 5 yanchi ACK，tongshirengjiexichu BotMessage。"""
    signing_key = SigningKey.generate()
    platform = _make_platform(signing_key.verify_key.encode().hex())
    payload = {
        "id": "interaction-1",
...
def test_format_response_wraps_interaction_callback():
    """type=2 jiaohuxiangyingyingshiyong Interaction Response huidiaogeshi（type=4 + data）。"""
...
def test_send_followup_missing_token_returns_false():
    """queshao interaction token shi send_followup yingfanhui False。"""
    from bot.models import BotMessage, BotResponse, ChatType
