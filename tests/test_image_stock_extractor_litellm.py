...
class TestCallLitellmVision:
    ...
    def test_raises_when_model_not_configured(self):
        ...
        with patch("src.services.image_stock_extractor.get_config", return_value=cfg):
            ...
            with pytest.raises(ValueError, match="weipeizhi Vision API"):
...
class TestParseItemsFromText:
    ...
    def test_parses_new_format(self):
        ...
        text = '[{"code":"600519","name":"guizhoumaotai","confidence":"high"},{"code":"00700","name":"tengxunkonggu","confidence":"medium"}]'

        items = _parse_items_from_text(text)

        assert len(items) == 2

        assert items[0] == ("600519", "guizhoumaotai", "high")

        assert items[1] == ("00700", "tengxunkonggu", "medium")
    ...
    def test_normalizes_invalid_confidence(self):
        ...
        text = '[{"code":"600519","name":"maotai","confidence":"invalid"}]'
    ...
    def test_filters_fake_codes_from_llm_field_names(self):
        ...
        text = '[{"code":"CODE","name":"field"},{"code":"159887","name":"ETF"},{"code":"NAME","name":"x"},{"code":"512880","name":"zhengquanETF"},{"code":"HIGH","name":"y"}]'
    ...
    def test_parses_markdown_wrapped_json_preserves_names(self):
        ...
        text = '\n\n```json\n[{"code":"159887","name":"yinhangETF","confidence":"high"},{"code":"512880","name":"zhengquanETF","confidence":"high"}]\n```'

        items = _parse_items_from_text(text)

        assert len(items) == 2

        assert items[0] == ("159887", "yinhangETF", "high")

        assert items[1] == ("512880", "zhengquanETF", "high")



    def test_uses_json_repair_when_json_invalid(self):

        text = '[{"code":"600519","name":"guizhoumaotai","confidence":"high"'

        items = _parse_items_from_text(text)

        assert items == [("600519", "guizhoumaotai", "high")]
...
class TestExtractStockCodesFromImage:
    ...
    def test_rejects_unsupported_mime(self):
        ...
        with pytest.raises(ValueError, match="buzhichidetupianleixing"):
    ...
    def test_rejects_empty_bytes(self):
        ...
        with pytest.raises(ValueError, match="tupianneirongweikong"):
    ...
    def test_wraps_litellm_error_message(self):
        ...
        with patch("src.services.image_stock_extractor.get_config", return_value=cfg), \
            ...
            with pytest.raises(ValueError, match="Vision API diaoyongshibai"):
