...
class TestAnalyzerGenerateText:
    ...
    def test_generate_text_returns_llm_response(self):
        ...
        with patch.object(analyzer, "_call_litellm", return_value="shichangfenxibaogao") as mock_call:

            result = analyzer.generate_text("xieyifenfupan", max_tokens=1024, temperature=0.5)

            assert result == "shichangfenxibaogao"
    ...
    def test_call_litellm_legacy_path_uses_legacy_model_list_for_param_recovery(self):
        ...
        with patch("src.analyzer.call_litellm_with_param_recovery", side_effect=_fake_call_litellm_with_param_recovery):
            ...
            text, _, _ = analyzer._call_litellm("huiguiyongli", {"max_tokens": 128, "temperature": 0.7})
    ...
    def test_analyze_integrity_retry_keeps_progress_monotonic(self):
        ...
        first_result = AnalysisResult(
            ...
            analysis_summary="shoulunjieguo",
        ...
        second_result = AnalysisResult(
            ...
            analysis_summary="buquanhoujieguo",
        ...
        with patch.object(analyzer, "is_available", return_value=True), \
            ...
            result = analyzer.analyze(
                ...
                {"code": "600519", "stock_name": "guizhoumaotai"},
        ...
        assert result.analysis_summary == "buquanhoujieguo"

        assert [progress for progress, _ in progress_updates] == [68, 93, 94, 95]

        assert "buquanzhongshi" in progress_updates[2][1]

        assert "jiexi JSON" in progress_updates[3][1]
    ...
    def test_parse_response_valid_json_returns_success(self):
        ...
        valid_response = json.dumps({
            ...
            "analysis_summary": "ceshifenxi",

        })

        result = GeminiAnalyzer._parse_response(analyzer, valid_response, "600519", "guizhoumaotai")
    ...
    def test_json_parse_failure_triggers_fallback_model(self):
        ...
        valid_json = _json.dumps({"sentiment_score": 70, "trend_prediction": "kanduo"})
        ...
        def fake_dispatch(model, call_kwargs, **kwargs):
            ...
            if "primary" in model:
                ...
                return SimpleNamespace(
                    ...
                    choices=[SimpleNamespace(message=SimpleNamespace(content="zhebushi JSON geshidexiangying"))],
    ...
    def test_all_models_invalid_json_raises_all_models_failed_error(self):
        ...
        assert exc_info.value.last_response_text == "zhebushi JSON geshidexiangying"
    ...
    def test_analyze_all_models_invalid_json_goes_through_post_processing(self):
        ...
        text_fallback_result = AnalysisResult(
            ...
            analysis_summary="bufenwenbenzhaiyao",
...
class TestMarketAnalyzerBypassFix:
    ...
    def _make_market_analyzer_with_mock_generate_text(self, return_value="fupanbaogao"):
    ...
    def test_no_access_to_private_model_attribute(self):
        ...
        ma = self._make_market_analyzer_with_mock_generate_text("fupanjieguo")
    ...
    def test_market_review_uses_8192_max_tokens(self):
        ...
        ma = self._make_market_analyzer_with_mock_generate_text(return_value="fupanjieguo")
    ...
    def test_generate_template_review_uses_english_shell_for_cn_when_report_language_is_en(self):
        ...
        overview = MarketOverview(
            ...
            top_sectors=[{"name": "AIsuanli", "change_pct": 3.25}],

            bottom_sectors=[{"name": "meitan", "change_pct": -1.12}],
...
"""
        ...
        assert "| 1 | AIsuanli | +3.25% |" in result

        assert "#### Lagging Sectors" in result

        assert "| 1 | meitan | -1.12% |" in result
...
"""
        ...
        assert "caozuojianyi" in result

        assert "panmianwendu" in result

        assert "| shangzhang/xiadie/pingpan | 3200 / 1800 / 100 |" in result

        assert "| zhishu | zuixin | zhangdiefu | kaipan | zuigao | zuidi | zhenfu | chengjiaoe(yi) |" in result

        assert "| shangzhengzhishu | 3300.00 | 🟢 +0.36% | 3288.00 | 3312.00 | 3276.00 | 1.10% | 1450 |" in result

        assert "#### lingzhangbankuai Top 5" in result

        assert "| 1 | AIsuanli | +3.25% |" in result

        assert "#### jinsanricuihuaxiansuo" in result

        assert "AIsuanlibankuaizouqiang" in result
    ...
    def test_news_block_labels_snippets_and_preserves_source_url(self):
        ...
        assert "zhaiyao/xiansuopianduan" in result

        assert "guanzhudian" not in result

        assert "chengjiaoefangda" in result

        assert "[dongfangcaifu / 2026-05-06](https://example.com/news/1)" in result
    ...
    def test_news_block_uses_dash_when_source_metadata_missing(self):
        ...
        result = ma._build_news_block([

            {
                ...
                "title": "zhengcelihaodaidongbankuaihuoyue",

                "snippet": "xiangguanzhutichengjiaofangda",
        ...
        assert "| 1 | zhengcelihaodaidongbankuaihuoyue | xiangguanzhutichengjiaofangda | - |" in result

        assert "| 1 | zhengcelihaodaidongbankuaihuoyue | xiangguanzhutichengjiaofangda | source |" not in result
    ...
    def test_market_light_snapshot_marks_defensive_market_red(self):
        ...
        overview = MarketOverview(
            ...
            indices=[
                ...
                MarketIndex(code="000001", name="shangzhengzhishu", current=3200, change_pct=-1.8),

                MarketIndex(code="399001", name="shenzhengchengzhi", current=9800, change_pct=-2.4),
        ...
        assert snapshot["label"] == "pianfangshou"

        assert snapshot["score"] < 40

        assert any("kuiqianxiaoying" in reason for reason in snapshot["reasons"])
    ...
    def test_indices_block_uses_configured_red_up_color_scheme(self):
        ...
        overview = MarketOverview(
            ...
            indices=[
                ...
                MarketIndex(code="000001", name="shangzhengzhishu", current=3200.0, change_pct=0.68),

                MarketIndex(code="399001", name="shenzhengchengzhi", current=9800.0, change_pct=-0.42),

                MarketIndex(code="399006", name="chuangyebanzhi", current=2100.0, change_pct=0.0),
        ...
        assert "| shangzhengzhishu | 3200.00 | 🔴 +0.68% |" in result

        assert "| shenzhengchengzhi | 9800.00 | 🟢 -0.42% |" in result

        assert "| chuangyebanzhi | 2100.00 | ⚪ +0.00% |" in result
