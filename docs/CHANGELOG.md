...
- [修复] 恢复 `REPORT_LANGUAGE=zh` 报告渲染兼容性，并保留分析 API 的 chart/event、analysis map、confidence 与 context pack overview 等结构化字段。
- [修复] CLI dry-run 在数据抓取成功时按成功统计，并在 dry-run 或无报告内容时跳过飞书云文档生成。
