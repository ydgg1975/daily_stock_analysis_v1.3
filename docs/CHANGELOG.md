...
- MiniMax LLM preset in Web UI channel editor (`LLMChannelEditor.tsx`) with M2.7 default model and `api.minimax.io` base URL
- MiniMax provider example in `litellm_config.example.yaml` and `.env.example` (`MINIMAX_LLM_API_KEY`)
- MiniMax LLM configuration section in `docs/LLM_CONFIG_GUIDE.md` and multilingual READMEs
