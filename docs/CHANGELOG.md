...
- [改进] `main.py` 在依赖缺失时输出中英双语的 "pip install -r requirements.txt" 提示并以 exit code 1 退出，取代原始 ModuleNotFoundError traceback，降低新贡献者的首次运行摩擦
- [改进] Web 前端 `NotFoundPage` 显示用户实际访问的路径（含 query），方便识别是 URL 拼写错误还是失效链接，取代原先只有通用"页面未找到"文案的体验
- [文档] README 快速开始新增"最小化可运行配置"示例（一个 AI Key + 一个推送渠道），置于庞大的 Secrets 表格之前，降低首次阅读负担
