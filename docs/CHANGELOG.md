...
- 💬 **Slack 一等通知渠道** — 新增 Slack 原生通知支持，同时支持 Incoming Webhook 和 Bot Token 两种接入方式；Bot Token 模式支持图片上传；新增 `SLACK_WEBHOOK_URL`、`SLACK_BOT_TOKEN`、`SLACK_CHANNEL_ID` 配置项，GitHub Actions 工作流同步补齐对应 Secrets 传递。
