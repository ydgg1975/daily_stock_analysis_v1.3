...
- [新功能] MiniMax 渠道模型列表升级：新增 `MiniMax-M3`（512K 上下文，128K 最大输出，支持图片输入）并作为默认；保留 `MiniMax-M2.7` 与 `MiniMax-M2.7-highspeed`；移除 `MiniMax-M2.5`。Web 设置页 MiniMax 预设模型与价格按 M3（$0.6/M 输入，$2.4/M 输出）刷新。
