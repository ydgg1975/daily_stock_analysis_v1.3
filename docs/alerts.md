...
이 문서는 Issue #1202에서 시작된 알림 센터의 실행 기준, 데이터 계약, 단계별 구현 범위, 호환 경계를 정리합니다.
...
- 규칙 소스: worker는 DB에 저장된 active rules를 읽고, legacy `AGENT_EVENT_ALERT_RULES_JSON`도 계속 지원합니다.
- 알림 전송: 트리거는 `NotificationService.send(..., route_type="alert")`를 사용하며 기존 alert 라우팅 설정을 따릅니다.
- 설정 검증: `src/services/system_config_service.py`가 legacy JSON 형식과 규칙 값을 검증합니다.

현재 실행 가능한 기본 규칙은 세 가지입니다.
...
`sentiment_shift`, `risk_flag`, `custom` 값은 향후 확장 자리이며 현재 실행 가능한 규칙으로 취급하지 않습니다.
...
- schedule 모드는 legacy 규칙이 없어도 worker를 등록해야 하며, API에서 새로 만든 active rule을 재시작 없이 평가할 수 있어야 합니다.
- Web/System 설정 저장은 JSON 형식, 필수 필드, 방향, 임계값, 지원 규칙 타입을 엄격히 검증해야 합니다.
- 로딩 중 단일 규칙이 잘못되면 그 규칙만 건너뛰고 나머지 규칙은 계속 평가합니다.
- worker는 프로세스 내부 fingerprint로 반복 발송을 줄입니다. 영속 평가 모델이나 재시작 후 유지되는 상태가 아닙니다.
...
알림 지속화를 구현할 때도 같은 구조를 재사용합니다. storage에는 ORM 모델, repository에는 CRUD와 조회, service에는 규칙 검증, 평가 상태, 알림 결과, 쿨다운 의미를 둡니다.

schema 변경 PR은 다음을 함께 설명해야 합니다.

- 중복 초기화가 기존 데이터를 깨지 않는지
- 알림 센터가 꺼져 있을 때 단일 분석, 문의, 알림, 시장 리뷰, 포트폴리오 기능에 영향이 없는지
- revert 전에 DB 테이블 또는 데이터 정리가 필요한지
...
- DB 테이블 추가
- 트리거 이력 구현
- `AGENT_EVENT_ALERT_RULES_JSON` 자동 마이그레이션, 삭제 또는 덮어쓰기
...
- MACD, KDJ, CCI, RSI 같은 기술 지표 규칙 추가
- legacy 설정 자동 마이그레이션 또는 legacy 설정 직접 편집 UI 제공

P1에서 추가한 `alert_rules` / `alert_triggers` / `alert_notifications` SQLite 테이블은 `Base.metadata.create_all()`로 생성됩니다. revert 전에 SQLite 테이블과 데이터를 자동 삭제하지 않으며, 보존 필요성을 확인한 뒤 수동으로 정리해야 합니다. P1 신규 Alert API 코드를 되돌리는 PR은 API 라우터 등록, schema, repository, service 참조를 함께 제거해야 합니다.

### P2 알림 평가 Worker

- schedule 경로를 DB active rule과 legacy JSON 규칙을 함께 평가하는 worker로 정리합니다.
...
- 정상 `not_triggered` 상태를 과도하게 쌓지 않기 위해 기록하지 않습니다.
- 알림 전송 per-channel attempt는 아직 `alert_notifications`에 쓰지 않습니다.
- 쿨다운은 `cooldown_policy`를 해석하지만 영속 쿨다운 상태는 만들지 않습니다.

### P3 Web 알림 센터 MVP

- WebUI에 `/alerts` 알림 센터를 추가합니다.
- 규칙 목록, 페이지네이션, 활성 필터, 규칙 대상 필터를 제공합니다.
- 생성 폼은 현재 실행 가능한 기본 세 가지 규칙과 `single_symbol` 대상을 우선 지원합니다.
- dry-run 테스트는 API schema에 선언된 값만 표시합니다.
- 알림 시도 이력은 `GET /api/v1/alerts/notifications`를 조회하지만, P2에서 per-channel attempt를 쓰지 않으면 빈 상태를 정상으로 봅니다.
- Web은 legacy JSON 직접 편집, 자동 마이그레이션, 자동 삭제를 제공하지 않습니다.

### P4 알림 결과와 영속 쿨다운

- 실제 트리거별 알림 전송 결과를 `alert_notifications`에 기록합니다.
- 비전송 상태도 synthetic channel로 기록할 수 있습니다. 예: `__cooldown__`, `__noise_suppressed__`, `__no_channel__`, `__dispatch__`
- DB 규칙은 `alert_cooldowns`로 영속 업무 쿨다운을 관리합니다.
- legacy JSON 규칙은 기존 worker 프로세스 내부 fingerprint를 계속 사용합니다.
- Web은 쿨다운 상태와 알림 결과를 읽기 전용으로 보여주며 쿨다운 정책 편집 UI는 제공하지 않습니다.
...
- 현재 level이 이미 조건을 만족하더라도 새 돌파가 아니면 `not_triggered`로 봅니다.
- 장중 partial bar는 보수적으로 제외합니다.
- MA, RSI, MACD, KDJ, CCI는 `src/services/alert_indicators.py`에서 OHLCV를 정규화해 직접 계산합니다.
- 샘플 부족, 필수 컬럼 누락, 데이터 소스 오류는 `degraded` 또는 `failed`로 기록하고 알림은 보내지 않습니다.
- `compute_required_bars`, `requested_days`, `required_bars > 365`, `prev <= threshold < current`, `Wilder`, `SMMA`, `alpha=1/period`, `EMA(fast_period)`, `alpha=1/k_period`, `0.015 * mean_deviation` 기준을 문서와 구현에서 유지합니다.
- 기술 지표 규칙은 Alert API/Web에서만 생성합니다. legacy JSON에는 추가하지 않습니다.
- 잘못된 MACD 방향, KDJ 교차 방향, MA window, 음수 threshold는 HTTP 400 + `validation_error` 또는 HTTP 400 + `unsupported_alert_type`으로 거절합니다.
- P5 revert 후 이전 코드는 기술 지표 규칙을 unsupported type으로 건너뛰어야 하며 legacy 세 규칙 실행을 막아서는 안 됩니다.

## 하지 않는 일

- legacy `AGENT_EVENT_ALERT_RULES_JSON` 자동 마이그레이션
