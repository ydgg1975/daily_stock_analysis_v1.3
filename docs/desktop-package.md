...
- 桌面端后端不再强制 `SCHEDULE_ENABLED=false`：在 `系统设置 -> 定时调度` 保存 `SCHEDULE_ENABLED=true` 后，定时推送会在**当前桌面应用运行期内**生效，无需重启应用（运行期由 `RuntimeSchedulerService` 管理）。
- 支持多时间点：通过 `SCHEDULE_TIMES=09:20,12:30,15:10,18:00` 配置多个每日推送时间；留空则回退到单时间 `SCHEDULE_TIME`。
- 设置页“定时调度”卡片展示当前是否启用、有效时间点、下次执行、上次执行结果，并提供“立即执行一次”按钮。
- 默认行为不变：未显式开启时默认 `SCHEDULE_ENABLED=false`，桌面端不会自动定时推送。
- **依赖前提**：桌面端定时推送依赖桌面应用及其内置后端进程保持运行；**关闭桌面应用后不会继续推送**。如需服务器侧 7×24 定时，请使用 `python main.py --schedule` 或容器/CI 部署。
