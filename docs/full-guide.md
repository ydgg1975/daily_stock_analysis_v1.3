...
规则引擎基于 OHLCV 数据自动计算技术指标，匹配 22 条声明式规则，覆盖 4 个维度（技术面/趋势面/资金面/估值面），生成加权评分和信号标签。

### 规则维度与信号

| 维度 | 说明 | 规则数 |
|------|------|--------|
| 技术面 (technical) | 均线交叉、多头/空头排列、MACD 交叉、RSI 超买超卖 | 8 |
| 趋势面 (trend) | MA20 趋势方向、20/60 日新高新低、短期强势 | 7 |
| 资金面 (capital) | 量价配合（放量上涨/缩量下跌）、布林带突破 | 4 |
| 估值面 (valuation) | PE 百分位（偏低/偏高/适中） | 3 |

每条规则的信号类型：`bullish`（偏多）、`bearish`（偏空）、`warning`（注意）、`neutral`（中性）。

### dimension_summary 语义

分析结果中 `dimension_summary` 始终返回全部 4 个维度，每个维度包含 4 种信号的匹配计数：
...
即使没有规则匹配，维度仍会返回（计数全为 0）。

### REST API

**POST /api/v1/rules/analyze** — 单只分析
...
响应包含 `symbol`、`asset_type`、`total_score`（加权评分）、`matched_rules`（匹配的规则列表）、`dimension_summary`、`tags`（逗号分隔的规则名称）。

**POST /api/v1/rules/batch** — 批量分析（最多 10 只）
...
Agent 可通过 `analyze_rules` 工具调用规则引擎：

- 参数：`stock_code`（股票/ETF 代码，如 `600519`、`510300`、`AAPL`）
- 返回：`symbol`、`asset_type`、`total_score`、`matched_rules`（仅匹配的规则）、`dimension_summary`、`tags`

### 策略：rules_analysis

系统内置 `rules_analysis` 策略（`strategies/rules_analysis.yaml`），别名：规则、规则分析、技术规则、指标规则。该策略将规则引擎评分作为情绪分数调整依据：
