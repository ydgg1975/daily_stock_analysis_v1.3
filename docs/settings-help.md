...
- `SCHEDULE_ENABLED`、`SCHEDULE_TIME`、`SCHEDULE_TIMES`：运行了调度服务的进程（API/Web/Desktop serve 模式）在保存后会自动 reconcile 调度——按最新配置启停或重建（多时间）每日任务，无需重启；未运行调度服务的进程（例如纯 CLI 单次模式）保存这些字段不会自动创建 scheduler。`SCHEDULE_TIMES` 为逗号分隔的 `HH:MM` 列表（如 `09:20,12:30,15:10,18:00`），留空时回退到单时间 `SCHEDULE_TIME`。
- `SCHEDULE_RUN_IMMEDIATELY`：启动期调度行为，仅决定调度服务启动时是否立即执行一次；保存后不会对当前已运行的调度触发一次性执行。需要保存后或临时执行一次时，请使用设置页“定时调度”卡片的“立即执行一次”（run-now）。
